...
# TODO: Create or refresh hierarchy for adapter slot D2.
# Suggested hierarchy path: uz_digital_adapter/D2_adapter
...
# Suggested hierarchy path: uz_digital_adapter/D3_adapter
...
foreach adapter_parent_hier [list uz_digital_adapter uz_analog_adapter] {
  ...
  foreach pin [get_bd_pins -quiet ${adapter_hier_path}/*] {
    foreach net [get_bd_nets -quiet -of_objects $pin] {
      ...
      set net_name [get_property NAME $net]
      catch {disconnect_bd_net -net $net_name $pin}
    }
  }
...
proc uz_pw_trim_unused_upstream_mi_pins {smartconnect_path} {
  set smartconnect_cell [get_bd_cells -quiet $smartconnect_path]
  if {[llength $smartconnect_cell] == 0} {
    puts "WARNING: Cannot trim AXI master interfaces because SmartConnect was not found: $smartconnect_path"
    return
  }

  set mi_count [get_property CONFIG.NUM_MI $smartconnect_cell]
  while {$mi_count > 0} {
    set last_index [expr {$mi_count - 1}]
    set mi_pin_path [uz_pw_get_sc_mi_pin $smartconnect_path $last_index]
    set mi_pin [get_bd_intf_pins -quiet $mi_pin_path]
    if {[llength $mi_pin] == 0} {
      set mi_count $last_index
      uz_pw_set_property_dict_if_objects [list CONFIG.NUM_MI "$mi_count"] $smartconnect_cell $smartconnect_path
      continue
    }
    ...
    puts "Removing unused trailing AXI master interface $mi_pin_path"
    set mi_count $last_index
    uz_pw_set_property_dict_if_objects [list CONFIG.NUM_MI "$mi_count"] $smartconnect_cell $smartconnect_path
  }
}

proc uz_pw_sc_mi_index_from_pin_path {pin_path} {
  if {[regexp {M0*([0-9]+)_AXI$} $pin_path -> index]} {
    scan $index "%d" decimal_index
    return $decimal_index
  }
  return -1
}

proc uz_pw_disconnect_intf_pin_nets {pin_path} {
  set pin [get_bd_intf_pins -quiet $pin_path]
  if {[llength $pin] == 0} {
    return
  }
...
proc uz_pw_remove_upstream_mi_connection {smartconnect_path removed_mi_pin} {
  if {$removed_mi_pin eq ""} {
    return
  }
  ...
  set removed_index [uz_pw_sc_mi_index_from_pin_path $removed_mi_pin]
  if {$removed_index < 0} {
    puts "WARNING: Could not determine SmartConnect master index from $removed_mi_pin"
    return
  }
  ...
  uz_pw_disconnect_intf_pin_nets $removed_mi_pin

  if {$removed_index < $last_index} {
    set last_mi_pin [uz_pw_get_sc_mi_pin $smartconnect_path $last_index]
    set last_mi [get_bd_intf_pins -quiet $last_mi_pin]
    set last_peer ""

    if {[llength $last_mi] > 0} {
      foreach intf_net [get_bd_intf_nets -quiet -of_objects $last_mi] {
    ...
    if {$last_peer ne "" && [llength [get_bd_intf_pins -quiet $removed_mi_pin]] > 0 && [llength [get_bd_intf_pins -quiet $last_peer]] > 0} {
      puts "Moving AXI connection from $last_mi_pin to $removed_mi_pin before reducing SmartConnect master interfaces"
      connect_bd_intf_net [get_bd_intf_pins $removed_mi_pin] [get_bd_intf_pins $last_peer]
    }
  }

  set new_mi_count [expr {$mi_count - 1}]
...
proc uz_pw_remove_slot_axi_attachment {slot} {
  ...
  set removed_mi_pin [uz_pw_find_peer_intf_pin $upstream_boundary_pin "*${::uz_pw_upstream_smartconnect}/M*_AXI"]
  uz_pw_remove_upstream_mi_connection $::uz_pw_upstream_smartconnect $removed_mi_pin
  ...
  uz_pw_trim_unused_upstream_mi_pins $::uz_pw_upstream_smartconnect
}
...
if {$uz_pw_upstream_smartconnect eq "" || [llength [get_bd_cells -quiet $uz_pw_upstream_smartconnect]] == 0} {
  ...
  uz_pw_remove_slot_axi_attachment D2

  uz_pw_remove_slot_axi_attachment D3
