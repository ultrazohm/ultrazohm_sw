...
- Each SPI Master (ADC chip) is able to read 16 single-ended or 8 differential channels.
  In order to interact with the :ref:`analog adapter board <Analog_MAX1131_12>`, the interface must be set to differential.
...
- The IP-Core instance needs the R5 processor to set the registers of the MAX11331 ADC chip for the following reasons:
    - Configure the SPI-Clock divider
    - Configure how many MAX11331 chips are connected (number of Masters). On :ref:`analog adapter board <Analog_MAX1131_12>` are three MAX11331 ADC chips, thus three Masters, used.
...
- Offset and conversion factor are configurable by software individually for each ADC in ``main.c``
  ...
  - Conversion factor is set to 10/6.65*3.3 = 4.96241 in default due to the amplifier circuit on :ref:`analog adapter board <Analog_MAX1131_12>`
