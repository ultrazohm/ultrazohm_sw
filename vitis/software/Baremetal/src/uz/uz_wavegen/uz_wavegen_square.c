...
#if UZ_WAVEGEN_SQUARE_MAX_INSTANCES > 0U
#include "uz_wavegen.h"
#include <stdint.h>
#include "../uz_HAL.h"

struct uz_wavegen_square {
    bool is_ready;
    struct uz_wavegen_phase_acc acc;
};

static uint32_t instance_counter = 0U;
static uz_wavegen_square_t instances[UZ_WAVEGEN_SQUARE_MAX_INSTANCES] = { 0 };

static uz_wavegen_square_t* uz_wavegen_square_allocation(void);

static uz_wavegen_square_t* uz_wavegen_square_allocation(void) {
    uz_assert(instance_counter < UZ_WAVEGEN_SQUARE_MAX_INSTANCES);
    uz_wavegen_square_t* self = &instances[instance_counter];
...
uz_wavegen_square_t* uz_wavegen_square_init(void) {
    uz_wavegen_square_t* self = uz_wavegen_square_allocation();
...
float uz_wavegen_square_sample(uz_wavegen_square_t* self, float amplitude, float frequency_Hz, float duty_cycle) {
    ...
    return (phase > duty_cycle) ? (-amplitude) : amplitude;
}
