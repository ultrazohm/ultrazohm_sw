...
float d_2[NUMBER_OF_OUTPUTS * NUMBER_OF_OUTPUTS] = {0};
//deltas
...
//
float gradtest1[21] = {
#include "functions_weights/GWB_test1.csv"
};
...
float T2[1] = {0}; // eigentlich nicht nötig da man cachebackprop im letzten layer nicht benötigt, aber fest definiert in layerconfig
...
void test_uz_nn_checkgradients_test1(void)
{
    ...
    uz_matrix_t* input=uz_matrix_init(&x_matrix,&x[0],2,1,NUMBER_OF_INPUTS);
    uz_nn_ff(test,input);
    uz_matrix_t* output=uz_nn_get_output_data(test);
    float result1=uz_matrix_get_element_zero_based(output,0,0);
    float error1 = reference_output[0]- result1;
    uz_nn_backward_pass(test,&error1,input);
    uz_matrix_t* gradhelptest1 = uz_nn_get_gradient_data(test,1); // index 1-2 verwenden für nn mit 2 layern
    uz_matrix_t* gradhelptest2 = uz_nn_get_gradient_data(test,2);
    float gradhelpertest1[UZ_MATRIX_SIZE(g_1)+UZ_MATRIX_SIZE(g_2)];
     ...
     for(int i=0;i< (int)(sizeof(gradhelpertest1) / sizeof(float));i++) {
