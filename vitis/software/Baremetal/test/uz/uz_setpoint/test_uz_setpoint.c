...
void test_uz_SetPoint_sample_NULL(void){
    TEST_ASSERT_FAIL_ASSERT(uz_SetPoint_sample(NULL, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, V_DC_Volts));
}

void test_uz_SetPoint_sample_V_DC_greater_than_zero(void){
    uz_SetPoint_t* instance = uz_SetPoint_init(config);
    TEST_ASSERT_FAIL_ASSERT(uz_SetPoint_sample(instance, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, 0.0f));
}
...
void test_uz_SetPoint_sample_MTPA_SMPMSM_operation_no_id(void){
    ...
    M_ref_Nm = 0.09f;
    output = uz_SetPoint_sample(instance, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, V_DC_Volts);
...
void test_uz_SetPoint_sample_MTPA_IPMSM_operation_limit_iq_id(void){
    ...
    id_ref_Ampere = 0.0f;
    output = uz_SetPoint_sample(instance, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, V_DC_Volts);
    TEST_ASSERT_FLOAT_WITHIN(1e-03, 14.7379f, output.q);
    TEST_ASSERT_FLOAT_WITHIN(1e-03,-2.7921f, output.d);
}
...
void test_uz_SetPoint_sample_field_weakening_SMPMSM_manual_id(void){
    ...
    M_ref_Nm = 0.09275f;
    omega_m_rad_per_sec = 418.75f;
    id_ref_Ampere = 5.0f; //Should be rejected, since its above the id_fw current
    uz_3ph_dq_t output = uz_SetPoint_sample(instance, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, V_DC_Volts);
    TEST_ASSERT_FLOAT_WITHIN(1e-03, 2.061f, output.q);
    TEST_ASSERT_FLOAT_WITHIN(1e-03, -5.007f, output.d);
    id_ref_Ampere = -10.0f; //Should be accepted, since its below the id_fw current
    output = uz_SetPoint_sample(instance, omega_m_rad_per_sec, M_ref_Nm, id_ref_Ampere, V_DC_Volts);
    TEST_ASSERT_FLOAT_WITHIN(1e-03, 2.061f, output.q);
