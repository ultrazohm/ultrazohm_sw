...
def render_available_machines_csv(
    entries: list[MachineCatalogEntry],
...
def write_available_machines_csv(
    ...
    output_path.write_text(
        render_available_machines_csv(entries, c_fields), encoding="utf-8", newline=""
    )
...
def main(argv: list[str] | None = None, *, default_anchor: str | Path | None = None) -> int:
    ...
    if args.check and (args.renumber or args.command is not None):
        parser.error("--check cannot be combined with --renumber or add_machine")

    if args.check:
        c_fields, entries = discover_machine_catalog(
            uz_pmsm_dir=args.uz_pmsm_dir, c_header_path=args.c_header_path,
        )
        return check_generated_artifacts(
            {
                args.inventory_output: render_available_machines_csv(entries, c_fields),
                args.header_output: render_c_init_header(
                    entries, c_fields=c_fields, source_root=args.uz_pmsm_dir,
                    generator_script="pyuzlib.machine_catalog",
                ),
            },
            regenerate_args=[
                "python3", "-m", "pyuzlib.machine_catalog",
                "--uz-pmsm-dir", str(args.uz_pmsm_dir.resolve()),
                "--c-header-path", str(args.c_header_path.resolve()),
                "--inventory-output", str(args.inventory_output.resolve()),
