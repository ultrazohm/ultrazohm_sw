...
.. tikz:: Schematic overview of the FrictionID
  ...
  \node [block, minimum width=9.52in,align=center,minimum height=5.2in,fill=blue!10](Online) {};
  \node[below,align=center]at(Online.north){\textbf{\LARGE{OnlineID}}};
  \node[block,below right=8em and 0.5em of Online.north west,minimum height= 3.25in, minimum width=1.5in](steady){};
  \node[below,align=center]at(steady.north)(eval){Evaluation of\\ steady states};
  \node[block,below left=9em and 0em of eval.north, align=center, anchor=north, minimum height=1em, text width=1.25in,fill=green!20](buffer){Save actual   values of\\ $i_*q$, $i_*d$, $u_*d$, $u_*q$ \& $\omega_*{el}$ in shift registers};
  ...
  \draw[<-]($(buffer.north)-(1em,0em)$)to node[right,align=left]{}($(Drehzahl.south)-(1em,0em)$);
  \node[block,text width=7.5em,minimum width=1.0in,right=10em of steady.north east, anchor=north west ](Stromgrenze){Evaluation, if \\ $  \sqrt{i_*d^2+i_*q^2} \leq I_*{lim}$};
  ...
  \node[block,above right=-2.8em and 10.45em of Stromgrenze.east,fill=green!20,text width=1.25in, minimum height=0.8in](inject){Inject\\ rectangle- \\signal $i_*{d,rec}$};
  \node[block,below right=5.65em and 1em of linParablock.south,text width=1.25in ,fill=yellow!20](linear){Identify $R_*s$, $L_*d$, $L_*q$, $\psi_*{PM}$ through measurement values from the signal injection};
  \node[block,right=1em of inject.east, text width=1.25in, minimum height=0.8in,fill=green!20 ](check){Evaluation, if\\steady state  \\has been left};
  ...
  \node[block, left=10em of linear.west,text width=1.25in ](flux){Calculate $\psi_*d$ and $\psi_*q$ from shift-\\registervalues  and fill $\Psi$ };
  ...
  \node[block,left=3em of flux, anchor=east, ](Diff){Calulate $\vartheta_*W $ and \\fill $\Delta \Psi$};
