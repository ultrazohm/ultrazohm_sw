...
class TclGenerator:
    ...
    def generate(
        ...
        warnings = self.validation_warnings(assignments, axi_config, option_values)
    ...
    def _axi_context(
        ...
        for slot in SLOTS:
            ...
            slot_axi = a_slot_axi if slot.startswith("A") else d_slot_axi
            adapter_root_hier = card.get("vivado", {}).get("adapter_parent_hier", self._adapter_root_hier(slot))
            adapter_clock_pin = card.get("vivado", {}).get("adapter_clock_pin", "clk")
            adapter_resetn_pin = card.get("vivado", {}).get("adapter_resetn_pin", "resetn")
            axi_slots.append(
                {
                    "slot": slot,
                    "slot_lower": slot.lower(),
    ...
    def _io_card_warnings(
        ...
        for slot, card_id in assignments.items():
            ...
            io_card = card.get("vivado", {}).get("io_card", {})
            directions = self._io_card_directions(io_card, slot_options)
            default_modes = io_card.get("default_modes", {})
            for index, physical_direction in enumerate(directions):
            ...
            if self._io_card_needs_axi(card, slot_options) and software_modes.get(slot) == "no_driver":
