...
uz_incrementalEncoder_t* initialize_incremental_encoder_ipcore_on_D4(float incrementalEncoderResolution, float motorPolePairNumber){
	struct uz_incrementalEncoder_config encoder_D4_config={
		.base_address=XPAR_UZ_USER_INCREMENTAL_ENCODER_0_BASEADDR,
		...
		.Encoder_elec_Offset = 0.0f,
		.counting_direction = uz_incrementalEncoder_counting_clock_wise,
		.Speed_Timeout_ms = 10U //10ms
	};
	return(uz_incrementalEncoder_init(encoder_D4_config));
}

void update_speed_and_position_of_encoder_on_D4(DS_Data* const data){	// update speed and position in global data struct
	data->av.theta_elec=uz_incrementalEncoder_get_theta_el(data->objects.encoder_D4);
	data->av.mechanicalRotorSpeed = uz_incrementalEncoder_get_omega_mech(data->objects.encoder_D4) * 60.0f / (2.0f*M_PI);
