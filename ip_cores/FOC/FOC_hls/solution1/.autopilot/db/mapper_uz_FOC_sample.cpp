...
extern "C" void uz_FOC_sample(int*, __cosim_s20__*, volatile void *, volatile void *, __cosim_sC__*, __cosim_sC__*, float, float, int, int);
extern "C" void apatb_uz_FOC_sample_hw(volatile void * __xlx_apatb_param_self, volatile void * __xlx_apatb_param_Controller_id, volatile void * __xlx_apatb_param_Controller_iq, __cosim_sC__ __xlx_apatb_param_i_reference_Ampere, __cosim_sC__ __xlx_apatb_param_i_actual_Ampere, float __xlx_apatb_param_V_dc_volts, float __xlx_apatb_param_omega_el_rad_per_sec, volatile void * __xlx_apatb_param_output_volts_d, volatile void * __xlx_apatb_param_output_volts_q) {
  ...
  vector<sc_bv<256> >__xlx_self__tmp_vec;
  for (int j = 0, e = 1; j != e; ++j) {
    sc_bv<256> _xlx_tmp_sc;
    _xlx_tmp_sc.range(63, 0) = ((long long*)__xlx_apatb_param_self)[j*4+0];
    _xlx_tmp_sc.range(127, 64) = ((long long*)__xlx_apatb_param_self)[j*4+1];
    _xlx_tmp_sc.range(191, 128) = ((long long*)__xlx_apatb_param_self)[j*4+2];
    _xlx_tmp_sc.range(255, 192) = ((long long*)__xlx_apatb_param_self)[j*4+3];
    __xlx_self__tmp_vec.push_back(_xlx_tmp_sc);
  }
  int __xlx_size_param_self = 1;
  int __xlx_offset_param_self = 0;
  int __xlx_offset_byte_param_self = 0*32;
  __cosim_s20__* __xlx_self__input_buffer= new __cosim_s20__[__xlx_self__tmp_vec.size()];
  for (int i = 0; i < __xlx_self__tmp_vec.size(); ++i) {
    ((long long*)__xlx_self__input_buffer)[i*4+0] = __xlx_self__tmp_vec[i].range(63, 0).to_uint64();
    ((long long*)__xlx_self__input_buffer)[i*4+1] = __xlx_self__tmp_vec[i].range(127, 64).to_uint64();
    ((long long*)__xlx_self__input_buffer)[i*4+2] = __xlx_self__tmp_vec[i].range(191, 128).to_uint64();
    ((long long*)__xlx_self__input_buffer)[i*4+3] = __xlx_self__tmp_vec[i].range(255, 192).to_uint64();
  }
  // DUT call
...
// print __xlx_apatb_param_self
  sc_bv<256>*__xlx_self_output_buffer = new sc_bv<256>[__xlx_size_param_self];
  for (int i = 0; i < __xlx_size_param_self; ++i) {
    char* start = (char*)(&(__xlx_self__input_buffer[__xlx_offset_param_self]));
    __xlx_self_output_buffer[i].range(63, 0) = ((long long*)start)[i*4+0];
    __xlx_self_output_buffer[i].range(127, 64) = ((long long*)start)[i*4+1];
    __xlx_self_output_buffer[i].range(191, 128) = ((long long*)start)[i*4+2];
    __xlx_self_output_buffer[i].range(255, 192) = ((long long*)start)[i*4+3];
  }
  for (int i = 0; i < __xlx_size_param_self; ++i) {
    ((long long*)__xlx_apatb_param_self)[i*4+0] = __xlx_self_output_buffer[i].range(63, 0).to_uint64();
    ((long long*)__xlx_apatb_param_self)[i*4+1] = __xlx_self_output_buffer[i].range(127, 64).to_uint64();
    ((long long*)__xlx_apatb_param_self)[i*4+2] = __xlx_self_output_buffer[i].range(191, 128).to_uint64();
    ((long long*)__xlx_apatb_param_self)[i*4+3] = __xlx_self_output_buffer[i].range(255, 192).to_uint64();
  }
}
