...
struct uz_delay_compensation_fcs_mpc_6phase_t {
    ...
    struct uz_delay_compensation_fcs_mpc_6phase_AXI_write_t AXI_write;
};
...
uz_delay_compensation_fcs_mpc_6phase_t* uz_delay_compensation_fcs_mpc_6phase_AXI_write(struct uz_delay_compensation_fcs_mpc_6phase_AXI_write_t AXI_write){
    uz_delay_compensation_fcs_mpc_6phase_t* self = uz_delay_compensation_fcs_mpc_6phase_allocation();
    uz_assert_not_zero_uint32(config.base_address);
    uz_assert_not_zero_uint32(config.ip_clk_frequency_Hz);
    self->config = config;

    uz_delay_compensation_fcs_mpc_6phase_pmsm_set_last_applied_optimal_voltage_ud(self->config.base_address, self->config.last_applied_optimal_voltage_ud);
