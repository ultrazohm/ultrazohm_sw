...
// extern bool is_three_phase_active; // from tutorial "Modifying the GUI"
...
void ipc_Control_func(uint32_t msgId, float value, DS_Data *data)
{
	...
	if (msgId != 0)
	{
		...
		case (Set_Send_Field_1):
			data->rasv.m_uf = value;
			//data->rasv.end_amplitude = value;
 			break;

		case (Set_Send_Field_2):
			data->rasv.m_ft = value;
			//data->rasv.end_frequency_Hz = value;
			break;

		case (Set_Send_Field_3):
			data->rasv.setpoint_RPM = value;
			//data->rasv.duration_sec = value;
			break;

		case (Set_Send_Field_4):
		    data->rasv.start_voltage = value;
			break;
		...
		case (Set_Send_Field_6):
			data->rasv.assertioncurrent = value;
			break;

		case (My_Button_1):
		//			Tutorial "Modifying the GUI"
		  Global_Data.rasv.is_three_phase_active = true;
	...
	/* Bit 4 - My_Button_1 */
	if (Global_Data.rasv.is_three_phase_active == true) {
	...
	/* Bit 5 - My_Button_2 */
	if (Global_Data.rasv.is_three_phase_active == false) {
	...
	/* tutorial "Modifying the GUI"
    if (!is_three_phase_active) { // from tutorial "Modifying the GUI"
       js_status_BareToRTOS |= 1 << 8;
    } else {
       js_status_BareToRTOS &= ~(1 << 8);
    }
