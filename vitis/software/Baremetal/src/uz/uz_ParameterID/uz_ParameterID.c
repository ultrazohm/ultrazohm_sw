...
uz_3ph_dq_t uz_ParameterID_Controller(uz_ParameterID_Data_t* Data, uz_CurrentControl_t* CC_instance_dq, uz_SpeedControl_t* SC_instance, uz_SetPoint_t* SP_instance) {
	uz_assert_not_NULL(Data);
	uz_assert_not_NULL(CC_instance_dq);
	...
	if (Data->Controller_Parameters.enableFOC_speed) {
		...
		uz_SpeedControl_set_ext_clamping(SC_instance, uz_CurrentControl_get_ext_clamping(CC_instance_dq));
		SpeedControl_reference_torque = uz_SpeedControl_sample(SC_instance, Data->ActualValues.omega_m, Data->Controller_Parameters.n_ref_FOC);
	...
	if (Data->Controller_Parameters.enableFOC_current || Data->Controller_Parameters.enableFOC_speed || Data->Controller_Parameters.enableFOC_torque) {
		...
		if (Data->Controller_Parameters.enableFOC_current) {
			...
			v_dq_Volts = uz_CurrentControl_sample(CC_instance_dq, Data->Controller_Parameters.i_dq_ref, Data->ActualValues.i_dq, Data->ActualValues.V_DC, Data->ActualValues.omega_el);
			...
			v_dq_Volts = uz_CurrentControl_sample(CC_instance_dq, i_SpeedControl_reference_Ampere, Data->ActualValues.i_dq, Data->ActualValues.V_DC, Data->ActualValues.omega_el);
		}
	}
	if (Data->Controller_Parameters.resetIntegrator) {
	...
	if (Data->ControlFlags->transNr > 0U && Data->ControlFlags->transNr <= 4U) {
		if (Data->Controller_Parameters.activeState == 144U) {
			uz_CurrentControl_set_decoupling_method(CC_instance_dq, no_decoupling);
		} else if (Data->Controller_Parameters.activeState == 170U) {
			uz_CurrentControl_set_decoupling_method(CC_instance_dq, linear_decoupling);
		}
		uz_CurrentControl_set_Kp_id(CC_instance_dq, Data->Controller_Parameters.Kp_id_out);
		uz_CurrentControl_set_Kp_iq(CC_instance_dq, Data->Controller_Parameters.Kp_iq_out);
		uz_CurrentControl_set_Ki_id(CC_instance_dq, Data->Controller_Parameters.Ki_id_out);
		uz_CurrentControl_set_Ki_iq(CC_instance_dq, Data->Controller_Parameters.Ki_iq_out);
		uz_SpeedControl_set_Ki(SC_instance, Data->Controller_Parameters.Ki_n_out);
		uz_SpeedControl_set_Kp(SC_instance, Data->Controller_Parameters.Kp_n_out);
	}
	...
	if (Data->ControlFlags->finished_all_Offline_states) {
		...
		if (Data->ParaID_Control_Selection == Current_Control || Data->ParaID_Control_Selection == Speed_Control || Data->ParaID_Control_Selection == Torque_Control) {
			if (Data->ParaID_Control_Selection == Speed_Control) {
				uz_SpeedControl_set_ext_clamping(SC_instance, uz_CurrentControl_get_ext_clamping(CC_instance_dq));
				SpeedControl_reference_torque = uz_SpeedControl_sample(SC_instance, Data->ActualValues.omega_m, Data->GlobalConfig.n_ref);
		...
		v_dq_Volts = uz_CurrentControl_sample(CC_instance_dq, Online_current_ref, Data->ActualValues.i_dq, Data->ActualValues.V_DC, Data->ActualValues.omega_el);
	}
	return (v_dq_Volts);
}
...
static void uz_ParameterID_initialize_data_structs(uz_ParameterID_t *self, uz_ParameterID_Data_t *Data) {
	...
	Data->GlobalConfig.PMSM_config.Ld_Henry = 0.5f;
	Data->GlobalConfig.PMSM_config.Lq_Henry = 1.0f;
	Data->GlobalConfig.PMSM_config.R_ph_Ohm = 1.0f;
	Data->GlobalConfig.PMSM_config.Psi_PM_Vs = 1.00f;
	Data->GlobalConfig.PMSM_config.polePairs = 5.0f;
	Data->GlobalConfig.PMSM_config.J_kg_m_squared = 1.0f;
	Data->GlobalConfig.PMSM_config.I_max_Ampere = 30.0f;
