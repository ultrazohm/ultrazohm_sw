...
// Instance management (only if we dont have the global_configuration.h included)
//#ifndef UZ_BLDC_CONTROL_MAX_INSTANCES
//#define UZ_BLDC_CONTROL_MAX_INSTANCES 1
//#endif
...
void uz_BLDC_control_sample(uz_BLDC_control* self, int sector, float U_ctrl_ref, uz_PWM_SS_2L_t* pwm_handle, float n_ref_rpm, float n_act_rpm, float I_ph_ref, float I_ph_m, float U_ZK, bool SpeedControl, bool CurrentControl, bool CascadeControl, bool DutyCycleControl) {
   ...
   float u_BLDC_ref;  // Assuming I want to calculate this here
	...
	if(SpeedControl){
		...
		n_RPM_error = n_ref_rpm - n_act_rpm;
		u_BLDC_ref = uz_PI_Controller_sample(self->PI_speed_only_mode, n_ref_rpm, n_act_rpm, false);

	}

	else if(CurrentControl){
		// ***Current Control (Current -> Control Voltage)
		I_ph_error = I_ph_ref - I_ph_m;
		u_BLDC_ref = uz_PI_Controller_sample(self->PI_current_mode, I_ph_ref, I_ph_m, false);
	}
		...
		n_RPM_error = n_ref_rpm - n_act_rpm;
		I_ph_error = i_ref_kaskade - I_ph_m;
		i_ref_kaskade = uz_PI_Controller_sample(self->PI_speed_mode, n_ref_rpm, n_act_rpm, false);
		u_BLDC_ref = uz_PI_Controller_sample(self->PI_current_mode, i_ref_kaskade, I_ph_m, false);
	}

	else if(DutyCycleControl){
		u_BLDC_ref = U_ctrl_ref;
	}

	// Now we normalize the control voltage from the current PI controller by dividing it by U_ZK
	d_BLDC = fabs(u_BLDC_ref)/U_ZK;
...
// Here we get/assign our duty cycles
