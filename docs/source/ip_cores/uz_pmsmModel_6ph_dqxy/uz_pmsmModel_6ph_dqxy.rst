.. _uz_pmsmModel_6ph_dqxy:

================================
6ph PMSM Model (dq-/xy-subspace)
================================

.. note:: This IP core is modified from :ref:`uz_pmsmModel` and extended by the xy-subspace.


- IP core of a 6ph PMSM model
- Simulates a 6ph PMSM on the FPGA
- Uses VSD modeling and the rotating dq- and xy-subspace
...
The model assumes a symmetric machine with a sinusoidal input voltage as well as VSD modeling.
Furthermore, only the dq- and xy-subspace(park-transformed with :math:`-\theta_{el}`) are modeled. 
...
.. math:: 
    ...
    \frac{d \psi_x}{dt} &= u_x - R_1 i_x - \omega_{el} \psi_y
...
Note, that the xy-subspace has no influence on the inner torque using this modeling approach.
...
.. tikz:: Block diagram of IP-Core
  ...
  \tikzstyle{block} = [draw, fill=black!10, rectangle, minimum height=3em, minimum width=3em]
  \node[name=ud]{$u_d$};
  \node[name=uq,below = 0.04cm of ud]{$u_q$};
  \node[name=ux,below = 0.04cm of uq]{$u_x$};
  \node[name=uy,below = 0.04cm of ux]{$u_y$};
  \node[draw,rectangle,fill=black!10,name=electrical,label=Electrical,below right= -0.45cm and 0.2cm of ud, minimum height=6em, minimum width=3em] {};
  ...
  \draw[->] ([xshift=-0.8cm,yshift=0.9 cm]electrical.west) -- ([yshift=0.9 cm]electrical.west);
  \draw[->] ([xshift=-0.8cm,yshift=0.3 cm]electrical.west) -- ([yshift=0.3 cm]electrical.west);
  \draw[->] ([xshift=-0.8cm,yshift=-0.3 cm]electrical.west) -- ([yshift=-0.3 cm]electrical.west);
  \draw[->] ([xshift=-0.8cm,yshift=-0.9 cm]electrical.west) -- ([yshift=-0.9 cm]electrical.west);
...
That is, :math:`u_d`, :math:`u_q`, :math:`u_x`, :math:`u_y`, :math:`\omega_{mech}`, and :math:`M_L` are inputs.
Furthermore, :math:`i_d`, :math:`i_q`, :math:`i_x`, :math:`i_y`, :math:`M_I`, and :math:`\omega_{mech}` are outputs.
The IP-Core inputs :math:`\boldsymbol{u}(k)=[{v}_{d} ~ v_{q} ~ v_{x} ~ v_{y} ~ T_{L}]` and outputs :math:`\boldsymbol{y}(k)=[i_{d} ~ i_{q} ~ i_{x} ~ i_{y} ~ T_{L} ~ \omega_{m}]` are accessible by AXI4 (including burst transactions).
...
.. math:: 
    ...
    \psi_x(k+1) &=t_s \bigg( u_x(k) - R_1 i_x(k) - \omega_{el} \psi_y(k) \bigg) + \psi_x(k)

    \psi_y(k+1) &=t_s \bigg( u_y(k) - R_1 i_y(k) + \omega_{el} \psi_x(k) \bigg) + \psi_y(k)
...
.. figure:: uz_pmsm_model_6ph_dqxy.svg
  :width: 700
  :align: center

  Test bench of PMSM plant model

.. figure:: uz_pmsm_model_6ph_dqxy_inside.svg
  :width: 700
  :align: center

  Overview of PMSM IP-Core

.. figure:: uz_pmsm_model_6ph_dqxy_inside_pmsm.svg
  :width: 700
  :align: center

  Calculation of PMSM subsystem

.. figure:: uz_pmsm_model_6ph_dqxy_inside_torque.svg
  :width: 700
  :align: center

  Torque calculation subsystem

.. figure:: uz_pmsm_model_6ph_dqxy_inside_mechanical.svg
  :width: 700
  :align: center
...
.. code-block:: c
  ...
  uz_pmsmModel_6ph_dqxy_t *pmsm_6ph=NULL;
  ...
  pmsm_6ph=uz_pmsmModel_6ph_dqxy_init(pmsm_config);
  // before ISR Init!
  // more code of main
...
.. code-block:: c
  ...
  uz_pmsmModel_6ph_dqxy_set_inputs(pmsm_6ph, pmsm_inputs);
  // [...]
  }
...
.. code-block:: c
  :caption: Adjust ``JavaScope_initalize`` function in ``javascope.c`` (R5) to measure pmsm_outputs
    ...
    extern struct uz_pmsmModel_6ph_dqxy_outputs_t pmsm_outputs;
    extern struct uz_pmsmModel_6ph_dqxy_inputs_t pmsm_inputs;
    ...
    js_ch_observable[JSO_i_x] = &pmsm_outputs.i_x_A;
    js_ch_observable[JSO_i_y] = &pmsm_outputs.i_y_A;
...
This code uses a deadtime of 1 ISR-clock.

.. code-block:: c

    uz_pmsmModel_6ph_dqxy_trigger_input_strobe(pmsm);
    uz_pmsmModel_6ph_dqxy_trigger_output_strobe(pmsm);
    pmsm_outputs=uz_pmsmModel_6ph_dqxy_get_outputs(pmsm);
    //Input your controller calculations here
    //Input = pmsm_outputs
    //Calculation
    //Output = pmsm_inputs
    uz_pmsmModel_6ph_dqxy_set_inputs(pmsm, pmsm_inputs);

Driver reference
================

.. doxygentypedef:: uz_pmsmModel_6ph_dqxy_t

.. doxygenstruct:: uz_pmsmModel_6ph_dqxy_config_t
  :members:

.. doxygenstruct:: uz_pmsmModel_6ph_dqxy_outputs_t
  :members:

.. doxygenstruct:: uz_pmsmModel_6ph_dqxy_inputs_t
  :members:  
  
.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_init

.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_set_inputs

.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_get_outputs

.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_reset

.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_trigger_input_strobe

.. doxygenfunction:: uz_pmsmModel_6ph_dqxy_trigger_output_strobe
