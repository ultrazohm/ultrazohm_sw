...
- Skips PL programming and PS initialization if the FPGA is already configured; otherwise behaves like the default modes (full PL programming and ``psu_init``).
...
- If the system state is inconsistent (e.g., after PL or platform changes), fall back to ``Run_UltraZohm`` or ``Debug_UltraZohm``.

Additional notes
----------------

- After initially building the Vitis workspace via ``vitis_generate_UltraZohm_workspace.tcl``, the launch configurations may only appear after restarting Vitis once.
- The programming sequence is defined by the TCL startup script ``tcl_scripts/vitis_debug_run_UltraZohm.tcl``.
