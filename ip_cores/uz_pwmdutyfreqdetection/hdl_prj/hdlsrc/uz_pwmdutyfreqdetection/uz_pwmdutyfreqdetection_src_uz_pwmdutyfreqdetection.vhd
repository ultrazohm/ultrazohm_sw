-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\uz_pwmdutyfreqdetection\uz_pwmdutyfreqdetection_src_uz_pwmdutyfreqdetection.vhd
-- Created: 2026-02-25 10:04:32
-- 
-- Generated by MATLAB 24.2, HDL Coder 24.2, and Simulink 24.2
-- 
-- 
-- -------------------------------------------------------------
-- Rate and Clocking Details
-- -------------------------------------------------------------
-- Model base rate: 1e-08
-- Target subsystem base rate: 1e-08
-- 
-- 
-- Clock Enable  Sample Time
-- -------------------------------------------------------------
-- ce_out        1e-08
-- -------------------------------------------------------------
-- 
-- 
-- Output Signal                 Clock Enable  Sample Time
-- -------------------------------------------------------------
-- AXI_output_0                  ce_out        1e-08
-- AXI_output_1                  ce_out        1e-08
-- AXI_output_2                  ce_out        1e-08
-- -------------------------------------------------------------
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: uz_pwmdutyfreqdetection_src_uz_pwmdutyfreqdetection
-- Source Path: uz_pwmdutyfreqdetection/uz_pwmdutyfreqdetection
-- Hierarchy Level: 0
-- Model version: 14.6
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;

ENTITY uz_pwmdutyfreqdetection_src_uz_pwmdutyfreqdetection IS
  PORT( clk                               :   IN    std_logic;
        reset                             :   IN    std_logic;
        clk_enable                        :   IN    std_logic;
        PWMin                             :   IN    std_logic;
        ce_out                            :   OUT   std_logic;
        AXI_output_0                      :   OUT   std_logic_vector(31 DOWNTO 0);  -- uint32
        AXI_output_1                      :   OUT   std_logic_vector(31 DOWNTO 0);  -- uint32
        AXI_output_2                      :   OUT   std_logic_vector(31 DOWNTO 0)  -- uint32
        );
END uz_pwmdutyfreqdetection_src_uz_pwmdutyfreqdetection;


ARCHITECTURE rtl OF uz_pwmdutyfreqdetection_src_uz_pwmdutyfreqdetection IS

  -- Component Declarations
  COMPONENT uz_pwmdutyfreqdetection_src_Detect_Rise_Positive
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          U                               :   IN    std_logic;
          Y                               :   OUT   std_logic
          );
  END COMPONENT;

  COMPONENT uz_pwmdutyfreqdetection_src_Detect_Fall_Nonpositive
    PORT( clk                             :   IN    std_logic;
          reset                           :   IN    std_logic;
          enb                             :   IN    std_logic;
          U                               :   IN    std_logic;
          Y                               :   OUT   std_logic
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : uz_pwmdutyfreqdetection_src_Detect_Rise_Positive
    USE ENTITY work.uz_pwmdutyfreqdetection_src_Detect_Rise_Positive(rtl);

  FOR ALL : uz_pwmdutyfreqdetection_src_Detect_Fall_Nonpositive
    USE ENTITY work.uz_pwmdutyfreqdetection_src_Detect_Fall_Nonpositive(rtl);

  -- Signals
  SIGNAL enb                              : std_logic;
  SIGNAL Delay7_out1                      : std_logic;
  SIGNAL Detect_Rise_Positive_out1        : std_logic;
  SIGNAL switch_compare_1                 : std_logic;
  SIGNAL Detect_Fall_Nonpositive_out1     : std_logic;
  SIGNAL switch_compare_1_1               : std_logic;
  SIGNAL HDL_Counter_ctrl_const_out       : std_logic;
  SIGNAL HDL_Counter_ctrl_delay_out       : std_logic;
  SIGNAL HDL_Counter_Initial_Val_out      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL count_step                       : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Logical_Operator_out1            : std_logic;
  SIGNAL count_from                       : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL count_reset                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL HDL_Counter_out1                 : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL count                            : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL need_to_wrap                     : std_logic;
  SIGNAL count_value                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL count_1                          : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL HDL_Counter_out                  : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Switch_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Delay1_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL switch_compare_1_2               : std_logic;
  SIGNAL Switch1_out1                     : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Delay3_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Sum1_out1                        : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Switch2_out1                     : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Delay6_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL period_counts                    : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL switch_compare_1_3               : std_logic;
  SIGNAL Switch3_out1                     : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Delay8_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL high_counts                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL switch_compare_1_4               : std_logic;
  SIGNAL Switch4_out1                     : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL Delay9_out1                      : unsigned(31 DOWNTO 0);  -- uint32
  SIGNAL low_counts                       : unsigned(31 DOWNTO 0);  -- uint32

BEGIN
  u_Detect_Rise_Positive : uz_pwmdutyfreqdetection_src_Detect_Rise_Positive
    PORT MAP( clk => clk,
              reset => reset,
              enb => clk_enable,
              U => Delay7_out1,
              Y => Detect_Rise_Positive_out1
              );

  u_Detect_Fall_Nonpositive : uz_pwmdutyfreqdetection_src_Detect_Fall_Nonpositive
    PORT MAP( clk => clk,
              reset => reset,
              enb => clk_enable,
              U => Delay7_out1,
              Y => Detect_Fall_Nonpositive_out1
              );

  enb <= clk_enable;

  Delay7_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay7_out1 <= '0';
      ELSIF enb = '1' THEN
        Delay7_out1 <= PWMin;
      END IF;
    END IF;
  END PROCESS Delay7_process;


  
  switch_compare_1 <= '1' WHEN Detect_Rise_Positive_out1 > '0' ELSE
      '0';

  
  switch_compare_1_1 <= '1' WHEN Detect_Fall_Nonpositive_out1 > '0' ELSE
      '0';

  HDL_Counter_ctrl_const_out <= '1';

  HDL_Counter_ctrl_delay_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        HDL_Counter_ctrl_delay_out <= '0';
      ELSIF enb = '1' THEN
        HDL_Counter_ctrl_delay_out <= HDL_Counter_ctrl_const_out;
      END IF;
    END IF;
  END PROCESS HDL_Counter_ctrl_delay_process;


  HDL_Counter_Initial_Val_out <= to_unsigned(1, 32);

  -- Count limited, Unsigned Counter
  --  initial value   = 1
  --  step value      = 1
  --  count to value  = 2500000
  count_step <= to_unsigned(1, 32);

  Logical_Operator_out1 <= Detect_Rise_Positive_out1 OR Detect_Fall_Nonpositive_out1;

  count_from <= to_unsigned(1, 32);

  count_reset <= to_unsigned(1, 32);

  count <= HDL_Counter_out1 + count_step;

  
  need_to_wrap <= '1' WHEN HDL_Counter_out1 = to_unsigned(2500000, 32) ELSE
      '0';

  
  count_value <= count WHEN need_to_wrap = '0' ELSE
      count_from;

  
  count_1 <= count_value WHEN Logical_Operator_out1 = '0' ELSE
      count_reset;

  HDL_Counter_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        HDL_Counter_out <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        HDL_Counter_out <= count_1;
      END IF;
    END IF;
  END PROCESS HDL_Counter_process;


  
  HDL_Counter_out1 <= HDL_Counter_Initial_Val_out WHEN HDL_Counter_ctrl_delay_out = '0' ELSE
      HDL_Counter_out;

  Delay1_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay1_out1 <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        Delay1_out1 <= Switch_out1;
      END IF;
    END IF;
  END PROCESS Delay1_process;


  
  Switch_out1 <= Delay1_out1 WHEN switch_compare_1_1 = '0' ELSE
      HDL_Counter_out1;

  
  switch_compare_1_2 <= '1' WHEN Detect_Rise_Positive_out1 > '0' ELSE
      '0';

  Delay3_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay3_out1 <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        Delay3_out1 <= Switch1_out1;
      END IF;
    END IF;
  END PROCESS Delay3_process;


  
  Switch1_out1 <= Delay3_out1 WHEN switch_compare_1_2 = '0' ELSE
      HDL_Counter_out1;

  Sum1_out1 <= Switch_out1 + Switch1_out1;

  Delay6_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay6_out1 <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        Delay6_out1 <= Switch2_out1;
      END IF;
    END IF;
  END PROCESS Delay6_process;


  
  Switch2_out1 <= Delay6_out1 WHEN switch_compare_1 = '0' ELSE
      Sum1_out1;

  Delay2_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        period_counts <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        period_counts <= Switch2_out1;
      END IF;
    END IF;
  END PROCESS Delay2_process;


  AXI_output_0 <= std_logic_vector(period_counts);

  
  switch_compare_1_3 <= '1' WHEN Detect_Rise_Positive_out1 > '0' ELSE
      '0';

  Delay8_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay8_out1 <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        Delay8_out1 <= Switch3_out1;
      END IF;
    END IF;
  END PROCESS Delay8_process;


  
  Switch3_out1 <= Delay8_out1 WHEN switch_compare_1_3 = '0' ELSE
      Switch_out1;

  Delay4_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        high_counts <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        high_counts <= Switch3_out1;
      END IF;
    END IF;
  END PROCESS Delay4_process;


  AXI_output_1 <= std_logic_vector(high_counts);

  
  switch_compare_1_4 <= '1' WHEN Detect_Rise_Positive_out1 > '0' ELSE
      '0';

  Delay9_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        Delay9_out1 <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        Delay9_out1 <= Switch4_out1;
      END IF;
    END IF;
  END PROCESS Delay9_process;


  
  Switch4_out1 <= Delay9_out1 WHEN switch_compare_1_4 = '0' ELSE
      Switch1_out1;

  Delay5_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        low_counts <= to_unsigned(0, 32);
      ELSIF enb = '1' THEN
        low_counts <= Switch4_out1;
      END IF;
    END IF;
  END PROCESS Delay5_process;


  AXI_output_2 <= std_logic_vector(low_counts);

  ce_out <= clk_enable;

END rtl;

