...
static uz_approximate_flux_q_t *uz_approximate_flux_q_allocation(void);

static uz_approximate_flux_q_t *uz_approximate_flux_q_allocation(void){
    uz_assert(instance_counter < UZ_APPROXIMATE_FLUX_Q_MAX_INSTANCES);
    uz_approximate_flux_q_t *self = &instances[instance_counter];
...
uz_approximate_flux_q_t* uz_approximate_flux_q_init(uz_PMSM_flux_fitting_parameter_config_t fitting_config){
    uz_approximate_flux_q_t* self = uz_approximate_flux_q_allocation();
