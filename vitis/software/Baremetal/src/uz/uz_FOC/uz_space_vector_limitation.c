...
static uz_dq_t uz_limit_dq_prio_d_axis(uz_dq_t v_input_Volts, float V_SV_max);
static uz_dq_t uz_limit_dq_prio_q_axis(uz_dq_t v_input_Volts, float V_SV_max);
uz_dq_t uz_FOC_SpaceVector_Limitation(uz_dq_t v_input_Volts, float V_dc_volts, float omega_el_rad_per_sec, uz_dq_t i_actual_Ampere, bool* ext_clamping){
	...
	uz_dq_t v_output_Volts = {0};
  	float V_SV_max =V_dc_volts / sqrtf(3.0f);
