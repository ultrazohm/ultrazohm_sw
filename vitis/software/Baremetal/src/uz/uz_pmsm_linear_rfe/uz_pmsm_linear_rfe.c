...
uz_3ph_dq_t uz_pmsm_linear_rfe_step(uz_pmsm_linear_rfe_t *self, uz_3ph_dq_t v, float omega_el){
    ...
    float id_dt=1/self->L_d*(v.d - (self->R + ((self->L_d*self->L_q*omega_el*omega_el)/self->rfe_q) ) * self->i_k.d + omega_el*self->L_q * self->i_k.q - ( (self->L_q * omega_el * omega_el)/self->rfe_q )*self->psi_pm  );
