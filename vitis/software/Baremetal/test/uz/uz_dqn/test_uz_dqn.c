...
//     [2] = {.activation_function = activation_linear, .number_of_neurons = NUMBER_OF_OUTPUTS, .number_of_inputs = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .length_of_weights = UZ_MATRIX_SIZE(tw_3), .length_of_bias = UZ_MATRIX_SIZE(tb_3), .length_of_output = UZ_MATRIX_SIZE(ty_3), .length_of_sumout = UZ_MATRIX_SIZE(ts_3), .weights = tw_3, .bias = tb_3, .output = ty_3, .sumout = ts_3}};
// // config critic
...
//         .error = e_1},
//     [1] = {.activation_function = activation_ReLU, .number_of_neurons = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_inputs = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_cachegradrows = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_cachegradcolumns = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_temporaryrows = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_temporarycolumns = NUMBER_OF_OUTPUTS, .length_of_weights = UZ_MATRIX_SIZE(cw_2), .length_of_bias = UZ_MATRIX_SIZE(cb_2), .length_of_output = UZ_MATRIX_SIZE(cy_2), .length_of_sumout = UZ_MATRIX_SIZE(cs_2), .length_of_delta = UZ_MATRIX_SIZE(delta_2), .length_of_gradients = UZ_MATRIX_SIZE(g_2), .length_of_error = UZ_MATRIX_SIZE(e_2), .length_of_temporarybackprop = UZ_MATRIX_SIZE(T2), .length_of_cachegradients = UZ_MATRIX_SIZE(cacheg_2), .weights = cw_2, .bias = cb_2, .output = cy_2, .sumout = cs_2, .delta = delta_2, .temporarybackprop = T2, .gradients = g_2, .cachegradients = cacheg_2, .error = e_2},
//     [2] = {.activation_function = activation_linear, .number_of_neurons = NUMBER_OF_OUTPUTS, .number_of_inputs = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_cachegradrows = NUMBER_OF_OUTPUTS, .number_of_cachegradcolumns = NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, .number_of_temporarycolumns = 2, .number_of_temporaryrows = 2, .length_of_weights = UZ_MATRIX_SIZE(cw_3), .length_of_bias = UZ_MATRIX_SIZE(cb_3), .length_of_output = UZ_MATRIX_SIZE(cy_3), .length_of_sumout = UZ_MATRIX_SIZE(cs_3), .length_of_delta = UZ_MATRIX_SIZE(delta_3), .length_of_gradients = UZ_MATRIX_SIZE(g_3), .length_of_error = UZ_MATRIX_SIZE(e_3), .length_of_temporarybackprop = UZ_MATRIX_SIZE(T3), .length_of_cachegradients = UZ_MATRIX_SIZE(cacheg_3), .weights = cw_3, .bias = cb_3, .output = cy_3, .sumout = cs_3, .delta = delta_3, .temporarybackprop = T3, .gradients = g_3, .cachegradients = cacheg_3, .error = e_3}};
// // config buffer
...
// // {
// //     uz_dqn_init(X_dat, lernrate, discountfact, config_critic, config_target, 2U, NUMBER_OF_HIDDEN_LAYER, configbuffer, EXPERIENCE_BUFFER_LENGTH,configenv);
// // }
...
//     uz_dqn_t *dqn = uz_dqn_init(X_dat, lernrate, discountfact, config_critic, config_target, 2U, NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, configbuffer, EXPERIENCE_BUFFER_LENGTH, configenv, MINIBATCHSIZE, TARGET_UPDATE_FREQUENCY, targsmoothfact);
...
// //     TEST_ASSERT_FLOAT_WITHIN(1e-03f, 64, loss);
// // }
...
// // {
// //     uz_dqn_t *testdqn = uz_dqn_init(X_dat, lernrate, discountfact, config_critic, config_target, 2U, NUMBER_OF_NEURONS_IN_HIDDEN_LAYER, configbuffer, EXPERIENCE_BUFFER_LENGTH, 0, configenv);
...
//     TEST_ASSERT_FAIL_ASSERT(epsilon_greedy_decay(epsilon_start, epsilon_min, epsilon_decay));
// }
...
//     {
//         float epsilon = epsilon_greedy_decay(epsilon_start, epsilon_min, epsilon_decay);
