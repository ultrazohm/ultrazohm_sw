...
static void write_config_to_pl(uz_pmsm_model6ph_dq_t *self);
...
uz_pmsm_model6ph_dq_t *uz_pmsm_model6ph_dq_init(struct uz_pmsm_model6ph_dq_config_t config){
    ...
    uz_pmsm_model6ph_dq_t *self = uz_pmsm_model6ph_dq_allocation();
    self->config = config;
...
void uz_pmsm_model6ph_dq_reset(uz_pmsm_model6ph_dq_t *self)
{
    ...
    uz_pmsm_model6ph_hw_write_reset(self->config.base_address, true);
    uz_sleep_useconds(1U);
    uz_pmsm_model6ph_hw_write_reset(self->config.base_address, false);
}
...
void uz_pmsm_model6ph_dq_set_inputs_general(uz_pmsm_model6ph_dq_t *self, float omega_mech, float load_torque){
    uz_assert_not_NULL(self);
    uz_assert(self->is_ready);
    uz_pmsm_model6ph_hw_write_omega_mech(self->config.base_address, omega_mech);
    uz_pmsm_model6ph_hw_write_load_torque(self->config.base_address, load_torque);
}

struct uz_pmsm_model6ph_dq_outputs_general_t uz_pmsm_model6ph_dq_get_outputs_general(uz_pmsm_model6ph_dq_t *self){
    uz_assert_not_NULL(self);
    uz_assert(self->is_ready);
    struct uz_pmsm_model6ph_dq_outputs_general_t outputs = {
        .torque = 0.0f,
        .omega_mech = 0.0f,
    	.theta_el = 0.0f};
    outputs.torque = uz_pmsm_model6ph_hw_read_torque(self->config.base_address);
    outputs.omega_mech = uz_pmsm_model6ph_hw_read_omega_mech(self->config.base_address);
    outputs.theta_el = uz_pmsm_model6ph_hw_read_theta_el(self->config.base_address);
    return outputs;
}
...
static void write_config_to_pl(uz_pmsm_model6ph_dq_t *self)
{
    uz_assert_not_NULL(self);
    uz_assert(self->is_ready);
    uz_pmsm_model6ph_hw_write_polepairs(self->config.base_address, self->config.polepairs);
    uz_pmsm_model6ph_hw_write_r_1(self->config.base_address, self->config.r_1);
...
void uz_pmsm_model6ph_trigger_voltage_input_strobe(uz_pmsm_model6ph_dq_t *self){
        uz_assert_not_NULL(self);
    uz_assert(self->is_ready);
    uz_pmsm_model6ph_trigger_voltage_input_strobe_hw(self->config.base_address);
}
