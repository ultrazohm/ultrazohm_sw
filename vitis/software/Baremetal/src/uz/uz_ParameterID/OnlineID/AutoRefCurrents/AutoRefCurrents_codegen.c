...
void AutoRefCurrents_step(RT_MODEL_AutoRefCurrents_t * const rtAutoRefCurrents_M)
{
	...
	ExtU_AutoRefCurrents_t *rtAutoRefCurrents_U = (ExtU_AutoRefCurrents_t *) rtAutoRefCurrents_M->inputs;
	ExtY_AutoRefCurrents_t *rtAutoRefCurrents_Y = (ExtY_AutoRefCurrents_t *) rtAutoRefCurrents_M->outputs;
   ...
   */
    ...
    /* . */
      ...
      /* '<S1>:23:1' sf_internal_predicateOutput = counter==(2.0/GlobalConfig.sampleTimeISR); */
			if (2.0F / rtAutoRefCurrents_U->GlobalConfig_out.sampleTimeISR == rtAutoRefCurrents_DW->counter) {
        /* Transition: '<S1>:23' */
        ...
        /* '<S1>:26:3' if(ActualValues.omega_m>0) */
          ...
          /* '<S1>:26:4' imax=abs(AutoRefCurrentsConfig.max_current); */
					rtAutoRefCurrents_DW->imax = fabsf(rtAutoRefCurrents_U->AutoRefCurrentsConfig.max_current);
        }
        ...
        /* '<S1>:26:15' if((i_dq_ref.q*i_dq_ref.q+(imax*idcount/AutoRefCurrentsConfig.id_points)^2)^0.5<abs(imax)) */
          ...
          /* '<S1>:26:16' i_dq_ref.d=-1*abs(imax)*idcount/AutoRefCurrentsConfig.id_points; */
					rtAutoRefCurrents_Y->i_dq_ref.d = -tmp * rtAutoRefCurrents_DW->idcount / rtAutoRefCurrents_U->AutoRefCurrentsConfig.id_points;
        } else {
          /* '<S1>:26:17' else */
          /* '<S1>:26:18' idcount=single(-AutoRefCurrentsConfig.id_points/3); */
					rtAutoRefCurrents_DW->idcount = -rtAutoRefCurrents_U->AutoRefCurrentsConfig.id_points / 3.0F;

          /* '<S1>:26:19' i_dq_ref.d=-1*abs(imax)*idcount/AutoRefCurrentsConfig.id_points; */
					rtAutoRefCurrents_Y->i_dq_ref.d = -fabsf(rtAutoRefCurrents_DW->imax) * rtAutoRefCurrents_DW->idcount / rtAutoRefCurrents_U->AutoRefCurrentsConfig.id_points;
        }
      ...
      /* '<S1>:22:3' if(iqcount<AutoRefCurrentsConfig.iq_points) */
			if (rtAutoRefCurrents_DW->iqcount < rtAutoRefCurrents_U->AutoRefCurrentsConfig.iq_points) {
      ...
      /* '<S1>:22:8' if(idcount<AutoRefCurrentsConfig.id_points) */
			if (rtAutoRefCurrents_DW->idcount < rtAutoRefCurrents_U->AutoRefCurrentsConfig.id_points) {
...
void AutoRefCurrents_initialize(RT_MODEL_AutoRefCurrents_t * const rtAutoRefCurrents_M)
{
	ExtY_AutoRefCurrents_t *rtAutoRefCurrents_Y = (ExtY_AutoRefCurrents_t *) rtAutoRefCurrents_M->outputs;
