...
typedef struct uz_CurrentControl_t
{
	...
	uz_3ph_dq_t flux_approx;
	uz_3ph_dq_t v_output_Volts;
	uz_3ph_dq_t v_pre_limit_Volts;
} uz_CurrentControl_t;
...
uz_3ph_dq_t uz_CurrentControl_sample(uz_CurrentControl_t *self, uz_3ph_dq_t i_reference_Ampere, uz_3ph_dq_t i_actual_Ampere, float V_dc_volts, float omega_el_rad_per_sec)
{
	...
	self->v_pre_limit_Volts = uz_CurrentControl_sample_pi_controllers(self, i_reference_Ampere, i_actual_Ampere);
	uz_3ph_dq_t v_decoup_Volts = uz_CurrentControl_decoupling(self, i_actual_Ampere, omega_el_rad_per_sec);
	self->v_pre_limit_Volts.d += v_decoup_Volts.d;
	self->v_pre_limit_Volts.q += v_decoup_Volts.q;
	self->v_output_Volts = uz_CurrentControl_SpaceVector_Limitation(self->v_pre_limit_Volts, V_dc_volts, self->config.max_modulation_index, omega_el_rad_per_sec, i_reference_Ampere, &self->ext_clamping);
	return (self->v_output_Volts);
}
...
static uz_3ph_dq_t uz_CurrentControl_sample_pi_controllers(uz_CurrentControl_t *self, uz_3ph_dq_t i_reference_Ampere, uz_3ph_dq_t i_actual_Ampere)
{
	uz_assert_not_NULL(self);
	uz_assert(self->is_ready);
	self->v_output_Volts.q = uz_PI_Controller_sample(self->Controller_iq, i_reference_Ampere.q, i_actual_Ampere.q, self->ext_clamping);
	self->v_output_Volts.d = uz_PI_Controller_sample(self->Controller_id, i_reference_Ampere.d, i_actual_Ampere.d, self->ext_clamping);
	return (self->v_output_Volts);
}
...
static uz_3ph_dq_t uz_CurrentControl_decoupling(uz_CurrentControl_t *self, uz_3ph_dq_t i_actual_Ampere, float omega_el_rad_per_sec)
{
	...
	case linear_decoupling:
		...
		break;
	case linear_decoupling_compensated:
		// Could also be estimated using
		// psi_d_estimated = (self->v_output_Volts.q - self->config.config_PMSM.R_ph_Ohm *i_actual_Ampere.q) / omega_el_rad_per_sec;
		// psi_q_estimated = (self->v_output_Volts.d - self->config.config_PMSM.R_ph_Ohm *i_actual_Ampere.d) / omega_el_rad_per_sec;
		float psi_d_k0 = self->config.config_PMSM.Ld_Henry * i_actual_Ampere.d + self->config.config_PMSM.Psi_PM_Vs;
		float psi_q_k0 = self->config.config_PMSM.Lq_Henry * i_actual_Ampere.q;
		float psi_d_k1 = psi_d_k0 + self->config.config_id.samplingTime_sec * (self->v_output_Volts.d - self->config.config_PMSM.R_ph_Ohm * i_actual_Ampere.d + omega_el_rad_per_sec * psi_q_k0);
		float psi_q_k1 = psi_q_k0 + self->config.config_id.samplingTime_sec * (self->v_output_Volts.q - self->config.config_PMSM.R_ph_Ohm * i_actual_Ampere.q - omega_el_rad_per_sec * psi_d_k0);
		float psi_d_k2 = psi_d_k1 + self->config.config_id.samplingTime_sec * self->v_pre_limit_Volts.d;
		float psi_q_k2 = psi_q_k1 + self->config.config_id.samplingTime_sec * self->v_pre_limit_Volts.q;
		decouple_voltage.d = -omega_el_rad_per_sec * 0.5f * (psi_q_k1 + psi_q_k2);
		decouple_voltage.q = omega_el_rad_per_sec * 0.5f * (psi_d_k1 + psi_d_k2);
		break;
