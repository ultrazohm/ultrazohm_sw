...
#define DDPG_PT1_EVAL_LOG_PATH "../../../workshop/logs/ceedling_eval_log_ddpg_module.csv"
...
void test_uz_ddpg_pt1_eval_module_export_to_csv(void)
{
    struct uz_ddpg_agent_config config = {
        .layers = ddpg_pt1_layer_config,
        .number_of_layers = DDPG_PT1_NUMBER_OF_LAYERS,
        .action_min = DDPG_PT1_ACTION_MIN,
        .action_max = DDPG_PT1_ACTION_MAX,
    };
    ...
    for (uint32_t episode_index = 0U; episode_index < DDPG_PT1_EVAL_EPISODES; episode_index++)
    {
        ...
        {
            const float action_value = uz_ddpg_agent_step(agent, y, reference);
            TEST_ASSERT_TRUE(action_value >= DDPG_PT1_ACTION_MIN);
            TEST_ASSERT_TRUE(action_value <= DDPG_PT1_ACTION_MAX);
