...
void uz_PWM_SS_2L_hw_SetDutyCycle(uint32_t base_address, float dutyCyc_A, float dutyCyc_B, float dutyCyc_C){
    ...
    uz_axi_write_uint32(base_address + m_u1_norm_AXI_Data_PWM_and_SS_control_V3_ip, (uint32_t)m_u1_norm);
    uz_axi_write_uint32(base_address + m_u2_norm_AXI_Data_PWM_and_SS_control_V3_ip, (uint32_t)m_u2_norm);
    uz_axi_write_uint32(base_address + m_u3_norm_AXI_Data_PWM_and_SS_control_V3_ip, (uint32_t)m_u3_norm);
}

void uz_PWM_SS_2L_hw_SetStatus(uint32_t base_address, uint32_t PWM_en){
    uz_assert_not_zero(base_address);
    uz_assert(PWM_en<=1);
    uz_axi_write_uint32(base_address + PWM_en_AXI_Data_PWM_and_SS_control_V3_ip, PWM_en);
}

void uz_PWM_SS_2L_hw_SetMode(uint32_t base_address, uint32_t PWM_mode){
    uz_assert_not_zero(base_address);
    uz_assert(PWM_mode<=2);
    uz_axi_write_uint32(base_address + Mode_AXI_Data_PWM_and_SS_control_V3_ip, PWM_mode);
}
...
void uz_PWM_SS_2L_hw_SetCarrierFrequency(uint32_t base_address, uint32_t ip_clk_frequency_Hz, float PWM_freq_Hz){
    ...
    uz_axi_write_uint32(base_address + Scal_f_carrier_AXI_Data_PWM_and_SS_control_V3_ip, PWM_Scal_f_carrier_Q26);
    ...
    uz_axi_write_uint32(base_address + Scal_T_carrier_AXI_Data_PWM_and_SS_control_V3_ip, (uint32_t)PWM_Scal_T_carrier);
}
...
void uz_PWM_SS_2L_hw_SetMinimumPulseWidth(uint32_t base_address, float min_pulse_width_percent){
    ...
    uz_axi_write_uint32(base_address + PWM_min_pulse_width_AXI_Data_PWM_and_SS_control_V3_ip, min_pulse_width_percent_Q12);
}
...
void uz_PWM_SS_2L_hw_SetTristate(uint32_t base_address, uint32_t halfBridgeNumber, uint32_t TriState_true_false){
    ...
    switch (halfBridgeNumber) {
        case 1  :
            halfBridgeAddress = TriState_HB1_AXI_Data_PWM_and_SS_control_V3_ip;
            break;

        case 2  :
            halfBridgeAddress = TriState_HB2_AXI_Data_PWM_and_SS_control_V3_ip;
            break;

        case 3  :
            halfBridgeAddress = TriState_HB3_AXI_Data_PWM_and_SS_control_V3_ip;
            break;
    }
