...
void update_speed_and_position_of_encoder_on_D5(DS_Data* const data){	// update speed and position in global data struct
	...
	data->av.theta_mech = uz_incrementalEncoder_get_position(data->objects.encoder_D5)/20000.0f * 2.0f*M_PI;

	data->av.theta_elec = data->av.theta_mech * 6.0f;

	data->av.theta_elec = fmod(data->av.theta_elec, 2.0f*M_PI);
