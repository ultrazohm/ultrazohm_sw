...
uz_Matrix_Multi_t* uz_Matrix_Multi_init(struct uz_Matrix_Multi_config config, volatile float * A,volatile float * B1,volatile float * B2,volatile float * C){
	...
	uz_matrix_multiplication_hw_set_A_rows(self->config.base_address, self->config.A_rows);//self->A_matrix->rows);
	uz_matrix_multiplication_hw_set_B1_matrix(self->config.base_address, self->config.B1_data);
	uz_matrix_multiplication_hw_set_B1_rows(self->config.base_address, self->config.B1_rows);//self->B1_matrix->rows);
