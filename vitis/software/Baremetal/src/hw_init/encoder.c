...
uz_incrementalEncoder_t* initialize_incremental_encoder_ipcore_on_D3(float incrementalEncoderResolution, float motorPolePairNumber){
	struct uz_incrementalEncoder_config encoder_D3_config={
		.base_address=XPAR_UZ_DIGITAL_ADAPTER_INCREMENTAL_ENCODER_0_BASEADDR,
	...
	return(uz_incrementalEncoder_init(encoder_D3_config));
}

void update_speed_and_position_of_encoder_on_D3(DS_Data* const data){	// update speed and position in global data struct
	data->av.theta_elec_incre=uz_incrementalEncoder_get_theta_el(data->objects.encoder_D3);
//	data->av.mechanicalRotorSpeed_incre = uz_incrementalEncoder_get_omega_mech(data->objects.encoder_D3) * 60.0f / (2.0f*M_PI)
