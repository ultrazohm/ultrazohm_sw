-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\IPCore_3Level_PWM\PWM_3Level_control_src_Subsystem_block.vhd
-- Created: 2025-11-17 13:37:05
-- 
-- Generated by MATLAB 24.1, HDL Coder 24.1, and Simulink 24.1
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: PWM_3Level_control_src_Subsystem_block
-- Source Path: IPCore_3Level_PWM/IP_Core_3-Level_PWM/GenPWM/Triangle_shift2/Subsystem
-- Hierarchy Level: 3
-- Model version: 1.13
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;
USE work.PWM_3Level_control_src_IP_Core_3_Level_PWM_pkg.ALL;

ENTITY PWM_3Level_control_src_Subsystem_block IS
  PORT( clk                               :   IN    std_logic;
        reset                             :   IN    std_logic;
        enb                               :   IN    std_logic;
        Zero_input                        :   IN    std_logic;
        converted_triangle_shift          :   IN    std_logic_vector(19 DOWNTO 0);  -- sfix20_En17
        modified_triangle_shift           :   IN    std_logic_vector(19 DOWNTO 0);  -- sfix20_En17
        triangle_in                       :   IN    std_logic_vector(19 DOWNTO 0);  -- sfix20_En17
        direction_down                    :   IN    std_logic;
        shifted_triangle_in_signal        :   OUT   std_logic_vector(20 DOWNTO 0);  -- sfix21_En17
        enable_out                        :   OUT   std_logic
        );
END PWM_3Level_control_src_Subsystem_block;


ARCHITECTURE rtl OF PWM_3Level_control_src_Subsystem_block IS

  -- Signals
  SIGNAL modified_triangle_shift_signed   : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL Unary_Minus_cast                 : signed(20 DOWNTO 0);  -- sfix21_En17
  SIGNAL Unary_Minus_cast_1               : signed(20 DOWNTO 0);  -- sfix21_En17
  SIGNAL Unary_Minus_out1                 : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL reduced_reg                      : vector_of_signed20(0 TO 39);  -- sfix20 [40]
  SIGNAL Unary_Minus_out1_1               : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL triangle_in_signed               : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL Sum8_sub_cast                    : signed(20 DOWNTO 0);  -- sfix21_En17
  SIGNAL Sum8_sub_cast_1                  : signed(20 DOWNTO 0);  -- sfix21_En17
  SIGNAL Sum8_out1                        : signed(20 DOWNTO 0);  -- sfix21_En17
  SIGNAL Relational_Operator11_out1       : std_logic;
  SIGNAL converted_triangle_shift_signed  : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL delayMatch2_reg                  : vector_of_signed20(0 TO 39);  -- sfix20 [40]
  SIGNAL converted_triangle_shift_1       : signed(19 DOWNTO 0);  -- sfix20_En17
  SIGNAL converted_triangle_shift_dtc     : std_logic;
  SIGNAL and2_1_out1                      : std_logic;
  SIGNAL and2_2_out1                      : std_logic;

BEGIN
  -- AND block replaced with 
  -- switches for better performance

  modified_triangle_shift_signed <= signed(modified_triangle_shift);

  Unary_Minus_cast <= resize(modified_triangle_shift_signed, 21);
  Unary_Minus_cast_1 <=  - (Unary_Minus_cast);
  Unary_Minus_out1 <= Unary_Minus_cast_1(19 DOWNTO 0);

  reduced_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        reduced_reg <= (OTHERS => to_signed(16#00000#, 20));
      ELSIF enb = '1' THEN
        reduced_reg(0) <= Unary_Minus_out1;
        reduced_reg(1 TO 39) <= reduced_reg(0 TO 38);
      END IF;
    END IF;
  END PROCESS reduced_process;

  Unary_Minus_out1_1 <= reduced_reg(39);

  triangle_in_signed <= signed(triangle_in);

  Sum8_sub_cast <= resize(Unary_Minus_out1_1, 21);
  Sum8_sub_cast_1 <= resize(triangle_in_signed, 21);
  Sum8_out1 <= Sum8_sub_cast - Sum8_sub_cast_1;

  shifted_triangle_in_signal <= std_logic_vector(Sum8_out1);

  
  Relational_Operator11_out1 <= '1' WHEN triangle_in_signed <= Unary_Minus_out1_1 ELSE
      '0';

  converted_triangle_shift_signed <= signed(converted_triangle_shift);

  delayMatch2_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset = '1' THEN
        delayMatch2_reg <= (OTHERS => to_signed(16#00000#, 20));
      ELSIF enb = '1' THEN
        delayMatch2_reg(0) <= converted_triangle_shift_signed;
        delayMatch2_reg(1 TO 39) <= delayMatch2_reg(0 TO 38);
      END IF;
    END IF;
  END PROCESS delayMatch2_process;

  converted_triangle_shift_1 <= delayMatch2_reg(39);

  
  converted_triangle_shift_dtc <= '1' WHEN converted_triangle_shift_1 /= to_signed(16#00000#, 20) ELSE
      '0';

  
  and2_1_out1 <= Zero_input WHEN Relational_Operator11_out1 = '0' ELSE
      converted_triangle_shift_dtc;

  
  and2_2_out1 <= Zero_input WHEN direction_down = '0' ELSE
      and2_1_out1;

  enable_out <= and2_2_out1;

END rtl;

