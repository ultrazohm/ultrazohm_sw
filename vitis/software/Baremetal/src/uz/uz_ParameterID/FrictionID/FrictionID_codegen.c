...
static void frictionID(ExtU_FrictionID_t *rtFrictionID_U, ExtY_FrictionID_t
  *rtFrictionID_Y, DW_FrictionID_t *rtFrictionID_DW);
  ...
  *rtFrictionID_Y, DW_FrictionID_t *rtFrictionID_DW)
{
  ...
  /* '<S1>:546:10' measArray2 			= single(zeros(2048,2)); */
  ...
  /* '<S1>:546:45' FrictionID_output.measArraySpeed			= single(zeros(256,1)); */
  /* '<S1>:546:46' FrictionID_output.measArrayTorque           = single(zeros(256,1)); */
  ...
  *rtFrictionID_Y, DW_FrictionID_t *rtFrictionID_DW)
{
  ...
  /* . */
  if (rtFrictionID_U->GlobalConfig_out.Reset ||
     ...
     case IN_CoulombFrictionTorqueEstimat:
      ...
      if (rtFrictionID_DW->nextstate == 1) {
        ...
        if (rtFrictionID_DW->is_CoulombFrictionTorqueEstimat == IN_DecreaseSpeed)
        {
          ...
          } else if ((rtFrictionID_DW->n_eva > 0.0F) &&
                     (!rtFrictionID_DW->switcher)) {
            /* '<S1>:488:10' elseif(n_eva>0&&switcher==boolean(0)) */
            /* '<S1>:488:11' switch_count = switch_count + 1; */
            qY = rtFrictionID_DW->switch_count + /*MW:OvSatOk*/ 1U;
            if (rtFrictionID_DW->switch_count + 1U <
                rtFrictionID_DW->switch_count) {
              qY = MAX_uint32_T;
            }
          ...
          /* '<S1>:488:13' if (n_eva>0 && switch_count>=5) */
          if ((rtFrictionID_DW->n_eva > 0.0F) && (rtFrictionID_DW->switch_count >=
               5U)) {
            /* '<S1>:488:14' n_eva=n_eva-(n_eva_step); */
            rtFrictionID_DW->n_eva -= rtFrictionID_DW->n_eva_step;

            /* '<S1>:488:15' switch_count = uint32(0); */
            rtFrictionID_DW->switch_count = 0U;
          }

          /* Inport: '<Root>/ActualValues' */
          /* '<S1>:488:17' if(abs(ActualValues.omega_m)<=omega_Brk&&counter>2) */
          if ((fabsf(rtFrictionID_U->ActualValues.omega_m) <=
               rtFrictionID_DW->omega_Brk) && (rtFrictionID_DW->counter > 2U)) {
            /* '<S1>:488:18' n_eva=single(0); */
            rtFrictionID_DW->n_eva = 0.0F;

            /* '<S1>:488:19' nextstate=uint16(1); */
...
void FrictionID_step(RT_MODEL_FrictionID_t *const rtFrictionID_M)
{
  ...
  if (rtFrictionID_DW->is_active_c7_FrictionID == 0U) {
    ...
    frictionID(rtFrictionID_U, rtFrictionID_Y, rtFrictionID_DW);
  }
