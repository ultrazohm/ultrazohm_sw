...
.. warning::
   * for each measurement mode, i.e., single-ended and fully differential, the ground of a possible external ADC power supply must be referenced to the ground of the UltraZohm. Otherwise, the analog front-end may be outside the voltage rails, resulting in offsets in the measured values.
   * due to missing pull-downs at the channel input of Rev02, unused channels are floating. This will be resolved in Rev03.

Compatibility 
-------------

* Slots A1 to A3 can be used without limitations. However, slot A3 is used for MAX1131 adapter board in the default UltraZohm software project.
...
Andreas Geiger (ZC), Eyke Aufderheide (TUM) in 11/2024
