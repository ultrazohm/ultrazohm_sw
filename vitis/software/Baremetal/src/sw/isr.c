...
bool reset = false;
extern XUz_pi_controller PI_instance_d;
extern XUz_pi_controller PI_instance_q;
...
void ISR_Control(void *data)
{
    ...
    if (current_state==control_state)
    {
        ...
        switch(openhw_pspl){
        case false:
        	XUz_pi_controller_Set_axi_referenceValue(&PI_instance_d, *int_idref);
			XUz_pi_controller_Set_axi_referenceValue(&PI_instance_q, *int_iqref);
        ...
        case true:
        	...
        	uz_PWM_SS_2L_set_duty_cycle(PWM1, duty_cycle.system1.DutyCycle_A, duty_cycle.system1.DutyCycle_B, duty_cycle.system1.DutyCycle_C);
        	uz_PWM_SS_2L_set_duty_cycle(PWM2, duty_cycle.system2.DutyCycle_A, duty_cycle.system2.DutyCycle_B, duty_cycle.system2.DutyCycle_C);
        	break;
	...
	if(reset){
		XUz_pi_controller_Set_axi_reset(&PI_instance_d, true);
		XUz_pi_controller_Set_axi_reset(&PI_instance_q, true);
		uz_CurrentControl_reset(cc_instance);
	}else{
		XUz_pi_controller_Set_axi_reset(&PI_instance_d, false);
		XUz_pi_controller_Set_axi_reset(&PI_instance_q, false);
	}
