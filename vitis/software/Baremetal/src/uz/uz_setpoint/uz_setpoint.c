...
uz_SetPoint_t* uz_SetPoint_init(struct uz_SetPoint_t_config config){
    ...
    self->config = config;
    uz_SetPoint_set_field_weakening(self, config.is_field_weakening_active);
    return(self);
}
...
void uz_SetPoint_set_field_weakening(uz_SetPoint_t* self, bool is_field_weakening_active) {
    ...
    if(is_field_weakening_active) {
        ...
        //Makes sure that, if IPMSM is selected, that a salience exists
        if(self->config.motor_type == IPMSM) {
            uz_assert(self->config.config_PMSM.Ld_Henry != self->config.config_PMSM.Lq_Henry);
        }
    }
...
switch (self->config.motor_type)
	{
        ...
        case (IPMSM):
            output.d = ((-self->config.config_PMSM.Psi_PM_Vs * self->config.config_PMSM.Ld_Henry) + (sqrtf(powf(self->config.config_PMSM.Psi_PM_Vs * self->config.config_PMSM.Ld_Henry, 2.0f) -
             (powf(self->config.config_PMSM.Ld_Henry, 2.0f) - powf(self->config.config_PMSM.Lq_Henry, 2.0f)) * 
             (powf(self->config.config_PMSM.Psi_PM_Vs, 2.0f) + (powf(self->config.config_PMSM.Lq_Henry, 2.0f) * powf(self->config.config_PMSM.I_max_Ampere, 2.0f)) - 
             (powf(V_SV_max, 2.0f) / powf(omega_el_rad_per_sec, 2.0f)))))) / (powf(self->config.config_PMSM.Ld_Henry, 2.0f) - powf(self->config.config_PMSM.Lq_Henry, 2.0f));
            break;
