...
struct uz_nn_layer_config software_nn_config[6] = {
	...
	[2] = {.activation_function = activation_ReLU, .number_of_neurons = NUMBER_OF_NEURONS_IN_FITH_LAYER, .number_of_inputs = NUMBER_OF_NEURONS_IN_FOURTH_LAYER, .length_of_weights = UZ_MATRIX_SIZE(w_5), .length_of_bias = UZ_MATRIX_SIZE(b_5), .length_of_output = UZ_MATRIX_SIZE(y_5), .weights = w_5, .bias = b_5, .output = y_5},
    //Note:Although IP-Core is hardcoded to activation_linear for the output the specified activation function for the last layer will be applied in software
