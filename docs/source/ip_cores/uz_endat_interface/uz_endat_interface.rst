...
The EnDat interface IP core connects EnDat absolute encoders to the UltraZohm.
...
7. It is highly recommended to also place an integrated logic analyzer block ``ILA`` and connect at least: ``trigger_endat_read``, ``endat_clk``, ``endat_data_in``, and ``endat_received_data_out_SH``, in order to debug the timing in case of high encoder clock frequencies or long encoder cables. See also :ref:`uz_endat_interface_delay_comp`.
...
.. important::
   ...
   The value of ``.sampling_interval_seconds`` must match the period of the ``trigger_endat_read`` signal that triggers the encoder communication.
...
.. hint::
   ...
   The PLL inside the EnDat IP core is based on the same implementation, but uses fixed-point math instead of floating-point math.
   This can reduce the PLL performance, especially at low speeds.
   If maximum PLL performance is required, use the SI position signals from the EnDat IP core and calculate the speed with the software implementation of the Position to Speed PLL instead.
...
.. note::
   ...
   In the example above this macro is ``XPAR_UZ_USER_UZ_ENDAT_INTERFACE_0_BASEADDR``.
...
The signal ``trigger_endat_read`` starts one EnDat transaction and writes the results of the previous EnDat transaction into the output registers.
For example, if ``trigger_endat_read`` is connected to ``trigger_conversions``, the position and speed values are updated once per control cycle.
...
``endat_transaction_done`` only indicates that an EnDat transaction is finished and does not indicate validity of the output values.
...
The figure shows the example for an EnDat encoder connected with a ``2 m`` encoder cable and a set clock frequency of ``2.5 MHz``.
...
Useful signals are ``trigger_endat_read``, ``endat_clk``, ``endat_data_in``, and ``endat_received_data_out_SH``.
