...
void error_checks_step(const actualValues *av, const error_checks_config_t *config)
{
    ...
    bool const ov_dc = av->IM_vdc > config->vdc_max;
    bool const oc_iu = fabsf(av->IM_ia) > config->iphase_max;
    bool const oc_iv = fabsf(av->IM_ib) > config->iphase_max;
    bool const oc_iw = fabsf(av->IM_ic) > config->iphase_max;
    bool const overspeed = fabsf(av->IM_mechanicalRotorSpeed_filtered) > config->max_mechanical_speed_rpm;
