...
data.Tsample = 1/20000;    % Frequenz Regelung
data.T_PWM = 1/20000;      % PWM Frequenz
data.TNi     = struct_PMSM_Config.Value.mot_Ld / struct_PMSM_Config.Value.mot_R1;
data.KPi = struct_PMSM_Config.Value.mot_R1 * data.TNi / (data.T_PWM + data.Tsample)+20;
data.TEi = data.T_PWM + data.Tsample;
data.TNn = 4 * data.TEi;
data.KPn = 0.5 * 2 * pi * (struct_PMSM_Config.Value.mot_J+struct_PMSM_In.Value.Last_J) / data.TEi +30;
data.n_hyst_upperlimit = struct_PMSM_Config.Value.mot_n_N/60/2/pi/100*1.0001; % Hysterese Drehzahlregler: 1% des Sollwerts
