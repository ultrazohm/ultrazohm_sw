...
//------------------------Parameter----------------------
`define TV_IN_u_dc "../tv/cdatafile/c.uz_SPWM_3ph.autotvin_u_dc.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter u_dc_DEPTH = 1;
reg [31 : 0] u_dc_OPERATE_DEPTH = 0;
parameter u_dc_c_bitwidth = 32;
parameter u_dc_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_u_dc [u_dc_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_u_dc [ (u_dc_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * u_dc_DEPTH -1 : 0] = '{default : 'hz};
reg u_dc_write_data_finish;
...
//write u_dc reg
reg [31 : 0] write_u_dc_count = 0;
reg write_u_dc_run_flag = 0;
reg write_one_u_dc_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & u_dc_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        u_dc_write_data_finish <= 0;
        write_u_dc_run_flag <= 0; 
        write_u_dc_count = 0;
        count_operate_depth_by_bitwidth_and_depth (u_dc_c_bitwidth, u_dc_DEPTH, u_dc_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_u_dc_run_flag <= 1; 
            write_u_dc_count = 0;
        end
        if (write_one_u_dc_data_done === 1) begin
            write_u_dc_count = write_u_dc_count + 1;
            if (write_u_dc_count == u_dc_OPERATE_DEPTH) begin
                write_u_dc_run_flag <= 0; 
                u_dc_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_u_dc
    integer write_u_dc_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] u_dc_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = u_dc_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_u_dc_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write u_dc data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (u_dc_c_bitwidth < 32) begin
                        u_dc_data_tmp_reg = mem_u_dc[write_u_dc_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < u_dc_c_bitwidth) begin
                                u_dc_data_tmp_reg[j] = mem_u_dc[write_u_dc_count][i*32 + j];
                            end
                            else begin
                                u_dc_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_u_dc[write_u_dc_count * four_byte_num  + i]!==u_dc_data_tmp_reg) begin
                    write (u_dc_data_in_addr + write_u_dc_count * four_byte_num * 4 + i * 4, u_dc_data_tmp_reg, write_u_dc_resp);
                    image_mem_u_dc[write_u_dc_count * four_byte_num + i]=u_dc_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_u_dc_data_done <= 1;
                @(posedge clk);
                write_one_u_dc_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_u_dc_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [127 : 0] token_tmp; 
  //reg [u_dc_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (u_dc_c_bitwidth , factor);
  fp = $fopen(`TV_IN_u_dc ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_u_dc); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < u_dc_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_u_dc [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
