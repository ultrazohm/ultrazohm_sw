...
void uz_mlp_three_layer_write_bias(uz_mlp_three_layer_ip_t* self,uint32_t parallel_pcu, uz_matrix_t *bias, uint32_t layer)
{
    ...
    size_t rows = uz_matrix_get_number_of_rows(bias);
    size_t columns = uz_matrix_get_number_of_columns(bias);
    for (size_t pcu = 1; pcu <= parallel_pcu; pcu++)
    {
        for (size_t neuron = 0; neuron < columns; neuron++)
        {
            for (size_t i = 0; i < rows; i++)
            {
                float bias_value = uz_matrix_get_element_zero_based(bias, i, neuron);
