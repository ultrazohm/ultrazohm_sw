...
figure
subplot(2,1,1)
plot(log.dut_id_set,log.dut_vd);
hold on
plot(log.dut_id_set,log.dut_vd_ref);
legend('ud','ud_ref');

subplot(2,1,2)
plot(log.dut_id_set,log.dut_vq);
hold on
plot(log.dut_id_set,log.dut_vq_ref);
legend('ud','ud_ref');

%%
figure
subplot(2,1,1)
plot(log.dut_iq,log.dut_vd,'x');
hold on
plot(log.dut_iq,log.dut_vd_ref,'x');
legend('ud','ud_ref');

subplot(2,1,2)
plot(log.dut_iq,log.dut_vq,'x');
hold on
plot(log.dut_iq,log.dut_vq_ref,'x');
legend('ud','ud_ref');
%%
...
figure
plot(no_q_current_groupedTable.mean_dut_id,(no_q_current_groupedTable.mean_psi_d-0.0083)./no_q_current_groupedTable.mean_dut_id); % 
