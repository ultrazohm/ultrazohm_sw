 ...
 *    '<S24>/Subsystem - pi//2 delay'
 *    '<S30>/Subsystem - pi//2 delay'
 */
static void Subsystempi2delay(uint8_T rtu_Enable, real_T rtu_dq, real_T rtu_dq_a,
  real_T rtu_wt, real_T *rty_alpha_beta, real_T *rty_alpha_beta_d)
{
  ...
  /* Outputs for Enabled SubSystem: '<S24>/Subsystem - pi//2 delay' incorporates:
  ...
  /* End of Outputs for SubSystem: '<S24>/Subsystem - pi//2 delay' */
}
  ...
  real_T rtu_wt, real_T *rty_alpha_beta, real_T *rty_alpha_beta_k)
{
  ...
  /* Outputs for Enabled SubSystem: '<S24>/Subsystem1' incorporates:
  ...
  *localDW)
{
  ...
  for (quadrant = 0; quadrant < 3; quadrant++) {
     ...
     *  Gain: '<S15>/Gain3'
     *  SignalConversion generated from: '<S15>/Gain3'
     */
   ...
   *  Product: '<S6>/Product'
   */
  localDW->Sum = localDW->error * rtu_Kp_Id + DiscreteTimeIntegrator;
   ...
   *  Switch: '<S10>/Switch'
   */
  if (localDW->Sum > localDW->U_max_dq) {
    localDW->Sum = localDW->U_max_dq;
  } else if (localDW->Sum < -localDW->U_max_dq) {
  ...
  /* Sum: '<S3>/Add' */
  localDW->Sum += rtb_Sum_c;

  /* Gain: '<S8>/Gain' incorporates:
   *  Gain: '<S20>/Gain'
   *  Switch: '<S4>/Switch'
   */
  rtb_Gain_tmp = 0.5774 * rtu_U_IC;
   ...
   *  Switch: '<S12>/Switch'
   */
  if (localDW->Sum > rtb_Gain_tmp) {
    rtb_Switch2 = rtb_Gain_tmp;
  } else if (localDW->Sum < -rtb_Gain_tmp) {
    /* Switch: '<S12>/Switch' incorporates:
     *  Gain: '<S8>/Gain1'
     */
    rtb_Switch2 = -rtb_Gain_tmp;
  } else {
    rtb_Switch2 = localDW->Sum;
  }
  ...
  if (rtu_flg_LimitUdUq > 0.0) {
    ...
    *rty_ud_soll = localDW->Sum;
  }

  /* End of Switch: '<S8>/Switch2' */

  /* Sum: '<S7>/Subtract' */
  localDW->Sum = rtu_iq_ref - rty_Iq_ist[1];
   ...
   *  Product: '<S7>/Product'
   */
  rtb_Sum_c = localDW->Sum * rtu_Kp_Iq + localDW->Gain_j;
  ...
  if (rtu_flg_LimitUdUq > 0.0) {
     ...
     *  Sum: '<S8>/Add'
     */
    localDW->Gain = sqrt(rtb_Gain_tmp * rtb_Gain_tmp - rtb_Switch2 * rtb_Switch2);
     ...
     *  Switch: '<S13>/Switch'
     */
    if (localDW->U_max_dq > localDW->Gain) {
      *rty_uq_soll = localDW->Gain;
    } else if (localDW->U_max_dq < -localDW->Gain) {
  ...
  if (rtu_flg_theta_el_compensation > 0.0) {
    localDW->U_max_dq = 7.5000000000000007E-5 * rtu_w_el;
  } else {
  ...
  /* Outputs for Enabled SubSystem: '<S24>/Subsystem1' */
  Subsystem1(1, *rty_ud_soll, *rty_uq_soll, localDW->U_max_dq, &localDW->Fcn_d,
             &localDW->Fcn1_o);

  /* End of Outputs for SubSystem: '<S24>/Subsystem1' */

  /* Outputs for Enabled SubSystem: '<S24>/Subsystem - pi//2 delay' */
  ...
  /* Outputs for Enabled SubSystem: '<S30>/Subsystem1' */
  Subsystem1(1, *rty_ud_soll, *rty_uq_soll, localDW->U_max_dq, &localDW->Fcn,
             &localDW->Fcn1);

  /* End of Outputs for SubSystem: '<S30>/Subsystem1' */

  /* Outputs for Enabled SubSystem: '<S30>/Subsystem - pi//2 delay' */
  Subsystempi2delay(localC->Compare_fz, *rty_ud_soll, *rty_uq_soll,
                    localDW->U_max_dq, &localDW->Fcn_b, &localDW->Fcn1_h);

  /* End of Outputs for SubSystem: '<S30>/Subsystem - pi//2 delay' */

  /* MATLAB Function: '<S21>/SpaceVectorModulation' incorporates:
   *  Switch: '<S30>/Switch'
   */
  /* MATLAB Function 'uz_codegen/CurrentController/Modulation/SpaceVectorModulation/SpaceVectorModulation': '<S29>:1' */
  /* '<S29>:1:3' a_U_on=0; */
  localDW->U_max_dq = 0.0;
  ...
  /* '<S29>:1:23' a = abs(u_alpha) + 0.5774*abs(u_beta); */
  rtb_Switch2 = fabs(localDW->Fcn);
  localDW->c = fabs(localDW->Fcn1);
  localDW->Switch1 = 0.5774 * localDW->c + rtb_Switch2;

  /* '<S29>:1:24' b = abs(u_alpha) - 0.5774*abs(u_beta); */
  rtb_Sum_c = rtb_Switch2 - 0.5774 * fabs(localDW->Fcn1);

  /* '<S29>:1:25' c = 1.1547*abs(u_beta); */
  localDW->c *= 1.1547;

  /* '<S29>:1:27' U_max = 0.6667 * U_ZK; */
  localDW->U_max = 0.6667 * rtu_U_IC;

  /* Maximum Voltage U_ZL/sqrt(3) */
  /* Sektor und Quadranten nach Quang Dittrich S32 */
  /* '<S29>:1:31' if(u_beta < 0) */
  if (localDW->Fcn1 < 0.0) {
    /* '<S29>:1:32' if (u_alpha <0) */
    if (localDW->Fcn < 0.0) {
      /* '<S29>:1:33' quadrant=3; */
      quadrant = 3;
  ...
  if (sector == 1) {
    ...
    /* Quang S.34 */
    /* '<S29>:1:80' a_U_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->U_max_dq = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:81' a_V_on = 0.5 - u1*0.5 + u2*0.5; */
    localDW->Gain = (0.5 - localDW->a_W_on * 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:82' a_W_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->a_W_on = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;
  }

  /* '<S29>:1:86' if ((sector == 2) && (quadrant == 1)) */
  if ((sector == 2) && (quadrant == 1)) {
    /* '<S29>:1:88' u1 = a/(U_max); */
    localDW->a_W_on = localDW->Switch1 / localDW->U_max;

    /* '<S29>:1:89' u2 = (-abs(u_alpha) + 0.5774*abs(u_beta))/(U_max); */
    localDW->u2 = (0.5774 * fabs(localDW->Fcn1) + -rtb_Switch2) / localDW->U_max;

    /* '<S29>:1:91' a_U_on = 0.5 + u1*0.5 - u2*0.5; */
    localDW->U_max_dq = (localDW->a_W_on * 0.5 + 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:92' a_V_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->Gain = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:93' a_W_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->a_W_on = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;
  }

  /* '<S29>:1:97' if ((sector == 2) && (quadrant == 2)) */
  if ((sector == 2) && (quadrant == 2)) {
    /* '<S29>:1:99' u1 = (-abs(u_alpha) + 0.5774*abs(u_beta))/(U_max); */
    localDW->a_W_on = (0.5774 * fabs(localDW->Fcn1) + -fabs(localDW->Fcn)) /
      localDW->U_max;

    /* '<S29>:1:100' u2 = a/(U_max); */
    localDW->u2 = localDW->Switch1 / localDW->U_max;

    /* '<S29>:1:102' a_U_on = 0.5 + u1*0.5 - u2*0.5; */
    localDW->U_max_dq = (localDW->a_W_on * 0.5 + 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:103' a_V_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->Gain = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:104' a_W_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->a_W_on = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;
  }

  /* '<S29>:1:108' if (sector == 3) */
  if (sector == 3) {
    /* '<S29>:1:110' u1 = c/(U_max); */
    localDW->a_W_on = localDW->c / localDW->U_max;

    /* '<S29>:1:111' u2 = b/(U_max); */
    localDW->u2 = rtb_Sum_c / localDW->U_max;

    /* '<S29>:1:113' a_U_on=  0.5 - u1*0.5 - u2*0.5; */
    localDW->U_max_dq = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:114' a_V_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->Gain = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:115' a_W_on = 0.5 - u1*0.5 + u2*0.5; */
    localDW->a_W_on = (0.5 - localDW->a_W_on * 0.5) + localDW->u2 * 0.5;
  }

  /* '<S29>:1:118' if (sector == 4) */
  if (sector == 4) {
    /* '<S29>:1:120' u1 = b/(U_max); */
    localDW->a_W_on = rtb_Sum_c / localDW->U_max;

    /* '<S29>:1:121' u2 = c/(U_max); */
    localDW->u2 = localDW->c / localDW->U_max;

    /* '<S29>:1:123' a_U_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->U_max_dq = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:124' a_V_on = 0.5 + u1*0.5 - u2*0.5; */
    rtb_Switch2 = localDW->a_W_on * 0.5 + 0.5;
    localDW->Gain = rtb_Switch2 - localDW->u2 * 0.5;

    /* '<S29>:1:125' a_W_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->a_W_on = rtb_Switch2 + localDW->u2 * 0.5;
  }

  /* '<S29>:1:129' if ((sector == 5) && (quadrant == 3)) */
  if ((sector == 5) && (quadrant == 3)) {
    /* '<S29>:1:131' u1 = a/(U_max); */
    localDW->a_W_on = localDW->Switch1 / localDW->U_max;

    /* '<S29>:1:132' u2 = (-abs(u_alpha) + 0.5774*abs(u_beta))/(U_max); */
    localDW->u2 = (0.5774 * fabs(localDW->Fcn1) + -fabs(localDW->Fcn)) /
      localDW->U_max;

    /* '<S29>:1:134' a_U_on = 0.5 - u1*0.5 + u2*0.5; */
    localDW->U_max_dq = (0.5 - localDW->a_W_on * 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:135' a_V_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->Gain = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:136' a_W_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->a_W_on = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;
  }

  /* '<S29>:1:140' if ((sector == 5) && (quadrant == 4)) */
  if ((sector == 5) && (quadrant == 4)) {
    /* '<S29>:1:142' u1 =(-abs(u_alpha) + 0.5774*abs(u_beta))/(U_max); */
    localDW->a_W_on = (0.5774 * fabs(localDW->Fcn1) + -fabs(localDW->Fcn)) /
      localDW->U_max;

    /* '<S29>:1:143' u2 =  a/(U_max); */
    localDW->u2 = localDW->Switch1 / localDW->U_max;

    /* '<S29>:1:145' a_U_on = 0.5 - u1*0.5 + u2*0.5; */
    localDW->U_max_dq = (0.5 - localDW->a_W_on * 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:146' a_V_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->Gain = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:147' a_W_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->a_W_on = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;
  }

  /* '<S29>:1:151' if (sector == 6) */
  if (sector == 6) {
    /* '<S29>:1:153' u1 = c/(U_max); */
    localDW->a_W_on = localDW->c / localDW->U_max;

    /* '<S29>:1:154' u2 = b/(U_max); */
    localDW->u2 = rtb_Sum_c / localDW->U_max;

    /* '<S29>:1:156' a_U_on = 0.5 + u1*0.5 + u2*0.5; */
    localDW->U_max_dq = (localDW->a_W_on * 0.5 + 0.5) + localDW->u2 * 0.5;

    /* '<S29>:1:157' a_V_on = 0.5 - u1*0.5 - u2*0.5; */
    localDW->Gain = (0.5 - localDW->a_W_on * 0.5) - localDW->u2 * 0.5;

    /* '<S29>:1:158' a_W_on = 0.5 + u1*0.5 - u2*0.5; */
    localDW->a_W_on = (localDW->a_W_on * 0.5 + 0.5) - localDW->u2 * 0.5;
  }

  /* '<S29>:1:161' if a_U_on > 1.0 */
  if (localDW->U_max_dq > 1.0) {
    /* '<S29>:1:162' a_U_on = 1.0; */
    localDW->U_max_dq = 1.0;
  }

  /* '<S29>:1:164' if a_V_on > 1.0 */
  if (localDW->Gain > 1.0) {
    /* '<S29>:1:165' a_V_on = 1.0; */
    localDW->Gain = 1.0;
  }

  /* '<S29>:1:167' if a_W_on > 1.0 */
  if (localDW->a_W_on > 1.0) {
    /* '<S29>:1:168' a_W_on = 1.0; */
    localDW->a_W_on = 1.0;
  }

  /* Switch: '<S4>/Switch' incorporates:
   *  MATLAB Function: '<S21>/SpaceVectorModulation'
   */
  /* '<S29>:1:171' u_a = a_U_on; */
  /* '<S29>:1:172' u_b = a_V_on; */
  /* '<S29>:1:173' u_c = a_W_on; */
  if (rtu_flg_SpaceVectorModulation > 0.0) {
    rty_ua[0] = localDW->U_max_dq;
    rty_ua[1] = localDW->Gain;
  ...
  *rty_Id_ist = rty_Iq_ist[0];
  ...
  localDW->DiscreteTimeIntegrator_PREV_U_j = localDW->Sum * rtu_Ki_Iq;
}
    ...
    switch (rtDW->is_c3_uz_codegen0) {
     case IN_InProcess:
      ...
      if (rtU->RESET == 1.0F) {
         ...
         *  Outport: '<Root>/ud'
         */
        /* '<S1>:3:5' [Ua_DutyCycle, Ub_DutyCycle, Uc_DutyCycle, iq_ist, id_ist, ud, uq] = CurrentController(ia, ib, ic, w_el, flg_PreCntr, theta_el, U_IC,id_ref,iq_ref,flgLimitUdUq, Kp_Iq, Ki_Iq, Kp_Id, Ki_Id, flg_SpaceVectorModulation, RESET, flg_deadTimeCompensation, flg_theta_el_compensation) */
        ...
        CurrentController(rtM, (real_T)rtU->ia, (real_T)rtU->ib, (real_T)rtU->ic,
                          ...
                          &rtY->Ub_DutyCycle, &rtY->Uc_DutyCycle, rtDW->Switch,
     ...
     case IN_Reset:
      ...
      if (rtU->RESET == 0.0F) {
        ...
        CurrentController(rtM, (real_T)rtU->ia, (real_T)rtU->ib, (real_T)rtU->ic,
                          ...
                          rtDW->Switch, &rtY->id_ist, &rtY->ud, &rtY->uq,
                          &rtConstB.CurrentController_o,
                          &rtDW->CurrentController_o);
