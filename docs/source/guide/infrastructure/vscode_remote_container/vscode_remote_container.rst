...
The performance of using the remote container in VS Code on Windows is relatively slow. To speed this up significantly we will set up the remote container and VS Code to run directly in WSL2.

.. note::
   For Windows users, this is highly recommended. Running VS Code on Windows, all unit tests took over **10 minutes**. On Ubuntu, it took **8.2 seconds**.
...
#. At the root level, create a new directory in which you will store all your repositories by typing in the Ubuntu shell.
...
#. Open the settings of Docker Desktop and enable under ``Resources -> WSL Integration`` the integration for the Ubuntu distribution.
...
#. To use the remote container with VS Code inside the Ubuntu distribution open the Ubuntu shell and navigate to the local repository.
...
.. note::
   Sourcetree cannot be natively installed in the Ubuntu distribution. The repository in the Ubuntu distro can be added to Sourcetree. However, this is not advised since the performance is bad.
   It is recommended to either use the Ubuntu shell with the standard git commands or the :ref:`GitLens extension <vscode_remote_container_gitlense>`.

Known issues in WSL
-------------------

.. note::
   If you have issues with your git credentials in WSL, it is possible to link your WSL git installation to the Windows Git Credential Manager. As explained here: https://stackoverflow.com/questions/45925964/how-to-use-git-credential-store-on-wsl-ubuntu-on-windows
...
.. note::
   On some machines the error ``error: RPC failed; curl 56 GnuTLS recv error (-24): Decryption has failed`` appears during ``git clone`` when connected via Wi-Fi. This can be related to an out-of-date Wi-Fi driver https://github.com/microsoft/WSL/issues/4253#issuecomment-799902628. If updating the driver does not solve the issue, try to connect via VPN or LAN as a workaround.
...
- ``cd docs``
- ``make livehtml``
- A new browser window opens with a live preview of the documentation
...
.. figure:: container_gitlense.gif
  ...
  Use GitLens to inspect commits on a file
