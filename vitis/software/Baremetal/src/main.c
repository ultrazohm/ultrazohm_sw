...
int main(void)
{
    ...
    while (1)
    {
        ...
        case infinite_loop:
        	...
        	// evaluation prüfen
            if(Global_Data.dqnp.number_of_episodes>NUMBER_OF_EPOCHS){
            if (evalcounter < NUMBER_OF_EVALS)
            {
            chain=start_eval;
            }
            if(Global_Data.dqnp.number_of_episodes >= (NUMBER_OF_EPOCHS+NUMBER_OF_EVALS)){
            chain=end_eval;
            }
            }
