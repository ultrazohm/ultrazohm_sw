...
f1 = @(a)psid_cross_real-(a(1)*((2*(a(2)^2)*d_current).*exp(-(a(2)*d_current).^2)*(1-exp(-(a(3)*q_current_T).^2)))+a(4)*((2*(a(5).^2)*d_current).*exp(-(a(5)*d_current).^2)*(1-exp(-(a(6)*q_current_T).^2)))+a(7)*((2*(a(8)^2)*d_current).*exp(-(a(8)*d_current).^2)*(1-exp(-(a(9)*q_current_T).^2)))+a(10)*((2*(a(11)^2)*d_current).*exp(-(a(11)*d_current).^2)*(1-exp(-(a(12)*q_current_T).^2))))
...
adq_red = lsqnonlin(f1,initial,[],[],options)
...
adq_zwei = lsqnonlin(f1,adq_red,[],[],options)
...
mehrtests = k1*(((2*(a_d4^2)*d_current).*exp(-(a_d4*d_current).^2))*(1-exp(-(a_q4*q_current_T).^2)))+k2*(((2*(a_d5^2)*d_current).*exp(-(a_d5*d_current).^2))*(1-exp(-(a_q5*q_current_T).^2)))+k3*(((2*(a_d6^2)*d_current).*exp(-(a_d6*d_current).^2))*(1-exp(-(a_q6*q_current_T).^2)))+k4*(((2*(a_d7^2)*d_current).*exp(-(a_d7*d_current).^2))*(1-exp(-(a_q7*q_current_T).^2)));

flux_d_fake = psidself_padded-mehrtests;
...
surf(d_current, q_current, mehrtests);
xlabel('D Current');
ylabel('Q Current');
zlabel('Flux_d');
...
surf(d_current, q_current, psiq_cross_real);
xlabel('D Current');
ylabel('Q Current');
zlabel('Flux_q');
legend;
