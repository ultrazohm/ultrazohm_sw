...
architecture Behavioral of spi_master is
    ...
    signal bitcounter                                                 : integer range 0 to W_LENGTH + 1; -- wenn bitcounter = Laenge --> alle Bits uebertragen
    signal tx_reg                                                     : std_logic_vector(W_LENGTH - 1 downto 0) := (others => '0');
    signal rx_reg                                                     : std_logic_vector(W_LENGTH - 1 downto 0) := (others => '0');
        ...
        if (reset = '1') then
            ...
            case spitxstate is
                when spi_stx =>
                    ...
                    bitcounter <= W_LENGTH;
                    sclk       <= CPOL;
                    if (enable = '1' and (CONTINUOUS_TRIGGERED_MODE = false or (CONTINUOUS_TRIGGERED_MODE = true and TRIGGER_TRANSMISSION = '1'))) then
                        spitxstate <= del_pre;
                    end if;
...
--              end if;
        ...
        if (spitxstate = spi_txactive and spiclkstate = shift and delay = 0 and (cpha = '0' or bitcounter /= W_LENGTH)) then
