    ...
    if (current_state==control_state)
    {
    	...
    	while(1) {
    	    ...
    	    if (is_done == true) {
    	    	...
    	    	//Xil_DCacheInvalidateRange((uint32_t)((uint32_t*)C_matrix),32);//->Invalidate wenn R5 liesst
    	    	//Xil_DCacheInvalidateLine((uint32_t)((uint32_t*)C_matrix));
    	    	C_test_matrix[0] = C_matrix[0];
    	    	C_test_matrix[1] = C_matrix[1];
    	    	C_test_matrix[2] = C_matrix[2];
    	    	C_test_matrix[3] = C_matrix[3];
    	    	break;
    	    }
    	}
