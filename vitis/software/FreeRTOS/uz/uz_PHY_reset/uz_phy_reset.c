...
int uz_phy_reset(void)
{
	...
	// enable apply reset
	uz_gpio_set_enable_output(&MIO_PHY_Reset, true);
	uz_gpio_write_pin(&MIO_PHY_Reset, false);

	// wait for some time as defined in data sheet
	usleep(phy_reset_high_time_us);

	// release reset and disable MIO pin
	uz_gpio_write_pin(&MIO_PHY_Reset, true);
	uz_gpio_set_enable_output(&MIO_PHY_Reset, false);
...
static void uz_phy_reset_init() {
	...
	uz_gpio_init(&MIO_PHY_Reset, &Gpio_inst, PHY_RESET_MIO_PIN, OUTPUT_PIN);
}
