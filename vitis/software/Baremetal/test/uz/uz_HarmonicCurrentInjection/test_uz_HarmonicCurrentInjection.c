...
static uz_3ph_dq_t HCI_sample_with_filter(
    uz_HarmonicCurrentInjection_t* instance,
    uz_3ph_dq_t i_ref_harmonic_Ampere,
    uz_3ph_abc_t i_abc_actual_Ampere,
    float V_dc_volts,
    ...
    float theta_el_rad)
{
    ...
    uz_3ph_dq_t i_dqn_filtered_Ampere =
        uz_HarmonicCurrentInjection_filter(instance, i_abc_actual_Ampere, theta_el_rad);

    // gefilterte dqn-Ströme an sample übergeben
    return uz_HarmonicCurrentInjection_sample(
        instance,
        i_ref_harmonic_Ampere,
...
void test_uz_HarmonicCurrentInjection_sample_abc_negative_order(void){
    ...
    for (int i = 0; i < (int)(sizeof(theta_el_rad) / sizeof(float)); i++)
    {
        ...
        uz_3ph_dq_t HCI_dq = HCI_sample_with_filter(instance, i_ref_harmonic_Ampere, i_abc_actual_Ampere, V_dc_volts, omega_el_rad_per_sec, theta_el_rad[i]);
