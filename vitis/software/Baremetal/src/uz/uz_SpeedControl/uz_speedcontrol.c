...
static struct uz_dq_t uz_SpeedControl_field_weakening(struct uz_PMSM_t config_PMSM, float omega_el_rad_per_sec, float U_zk_Volts, bool* fw_flag);
...
struct uz_dq_t uz_SpeedControl_sample(uz_PI_Controller* self, float omega_el_rad_per_sec, float n_ref_rpm, float U_zk_Volts, float id_ref_Ampere, struct uz_PMSM_t config_PMSM, bool ext_clamping){
    ...
    struct uz_dq_t i_field_weakening_Ampere = uz_SpeedControl_field_weakening(config_PMSM, omega_el_rad_per_sec, U_zk_Volts, &fw_flag);
    uz_PI_Controller_update_limits(self, i_field_weakening_Ampere.q, -i_field_weakening_Ampere.q);
	i_output_Ampere.d = uz_SpeedControl_decide_id_ref(config_PMSM.I_max_Ampere, id_ref_Ampere, i_field_weakening_Ampere.d, fw_flag);
...
static struct uz_dq_t uz_SpeedControl_field_weakening(struct uz_PMSM_t config_PMSM, float omega_el_rad_per_sec, float U_zk_Volts, bool* fw_flag){
    ...
    if (omega_el_rad_per_sec > omega_cut) {
	    ...
	    *fw_flag = false;
        output.d = 0.0f;
        output.q = config_PMSM.I_max_Ampere;
    }
    return(output);
}
...
static struct uz_dq_t uz_SpeedControl_calculate_fw_currents(struct uz_PMSM_t config_PMSM, float omega_el_rad_per_sec, float omega_cut, float U_SV_max) {
    ...
    struct uz_dq_t output = {0};
	output.d = (config_PMSM.Psi_PM_Vs / config_PMSM.Ld_Henry) * ((omega_cut / omega_el_rad_per_sec) - 1.0f);
	...
	float b_iq_fw = 2.0f * config_PMSM.R_ph_Ohm * config_PMSM.Psi_PM_Vs * omega_el_rad_per_sec;
