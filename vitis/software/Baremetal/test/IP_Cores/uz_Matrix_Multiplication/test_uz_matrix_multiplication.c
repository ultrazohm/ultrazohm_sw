...
uz_Matrix_Multi_t* successful_init(){ 
    ...
    uz_matrix_multiplication_hw_set_C_out_matrix_Expect(config.base_address, config.C_data);
    uz_Matrix_Multi_t* test_instance = uz_Matrix_Multi_init(config, &A_matrix, &B_matrix, &C_out_matrix);
...
void test_uz_Matrix_Multi_init(void) {
    successful_init();
}
...
void test_uz_Matrix_Multi_continue_calculation(void) {
    uz_Matrix_Multi_t* instance = successful_init();
