...
uz_3ph_dq_t step_dq_res(uz_CurrentControl_t* PI, uz_subspace_resonant_control* res, DS_Data* Data, uz_3ph_dq_t currents){
	uz_3ph_dq_t pi_out = uz_CurrentControl_sample(PI, zero_ref_dq, currents, Data->av.U_ZK, Data->av.omega_el);
	uz_3ph_dq_t res_out = uz_subspace_resonant_control_step_dq(res, zero_ref_dq, currents, Data->av.omega_el);
	uz_3ph_dq_t out = {
...
uz_9ph_dq_t step_controllers_PIR_PIR(DS_Data* Data, struct pointers_PIR_PIR objects){
	...
	// step controllers
	uz_3ph_dq_t dq_ref_PI = uz_CurrentControl_sample(objects.PI_dq, Data->rasv.dq_setpoints, Data->av.currents_dq, Data->av.U_ZK, Data->av.omega_el);
	uz_3ph_dq_t dq_ref_res = uz_subspace_resonant_control_step_dq(objects.RES_dq, zero_ref_dq, Data->av.currents_dq, Data->av.omega_el);
	uz_3ph_dq_t xy1_ref = step_dq_res(objects.PI_xy1, objects.RES_xy1, Data, Data->av.currents_xy1);
	uz_3ph_dq_t xy2_ref = step_dq_res(objects.PI_xy2, objects.RES_xy2, Data, Data->av.currents_xy2);
	uz_3ph_dq_t xy3_ref = uz_subspace_resonant_control_step_dq(objects.RES_xy3, zero_ref_dq, Data->av.currents_xy3, Data->av.omega_el);
	...
	uz_3ph_alphabeta_t XY3_ref_7 = uz_subspace_resonant_control_step_alphabeta(objects.RES_XY3, zero_ref_ab, Data->av.currents_XY3, Data->av.omega_el);
	// out
...
void reset_controllers_PIR_PIR(struct pointers_PIR_PIR objects){
	uz_subspace_resonant_control_reset(objects.RES_dq);
	uz_subspace_resonant_control_reset(objects.RES_xy1);
	uz_subspace_resonant_control_reset(objects.RES_xy2);
	uz_subspace_resonant_control_reset(objects.RES_xy3);
	uz_subspace_resonant_control_reset(objects.RES_XY3);
