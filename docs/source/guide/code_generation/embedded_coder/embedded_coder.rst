...
- Every discrete-time step is synchronous to the highest discrete-time step in the generated model
...
- Use the ``single`` data type in MATLAB/Simulink, which is a 32-bit float
...
``uz_codegen_step(uz_codegen *self)``
 steps the model one time step, i.e., all calculations inside the Simulink model are executed once.
 Has to be passed a pointer to a variable of type ``uz_codegen``.
...
18. Call the init function (``uz_codegen_init(&codegenInstance)``) inside main
19. Add ``extern uz_codegen codegenInstance`` to ``Baremetal/sw/isr.c`` to use the global inside the ISR
