...
#include "uz_myIP2.h"
#include "uz_myIP2_private.h" // Required to init an instance
...
void test_uz_myIP2_test_A_times_B_equals_C(void){
    ...
    int32_t c=uz_myIP2_multiply(test_ptr,a,b);
    TEST_ASSERT_EQUAL_INT32(a*b,c);
}

void test_uz_myIP2_no_multiplication_before_initialization_and_no_NULL_pointer_passed(void){
     uz_myIP2 test_instance_no_init={
        .base_address=TEST_BASE_ADDRESS
     };
    int32_t a=10;
    int32_t b=20;
 ...
 void test_uz_myIP2_test_ip_frq_not_zero(void){
 ...
 void test_uz_myIP2_test_base_address_not_zero(void){
     uz_myIP2 test_instance={
        .ip_clk_frequency_Hz=TEST_IP_CORE_FRQ
     };
 ...
 void test_uz_myIP2_test_double_init(void){
     ...
     uz_myIP2* test_ptr=uz_myIP2_init(&test_instance);
     TEST_ASSERT_EQUAL_PTR(test_ptr,&test_instance);
     TEST_ASSERT_FAIL_ASSERT(test_ptr=uz_myIP2_init(&test_instance));
 }
...
void test_uz_myIP2_test_init_assert_NULL(void)
{
    TEST_ASSERT_FAIL_ASSERT(uz_myIP2_init(NULL));
}
