...
#include "impl_modul_mpc_single.h"
...
static void abs_m(const real32_T x[4], real32_T y[4]);
...
static real32_T rt_hypotf_o(real32_T u0, real32_T u1, DW *rtDW);
...
static void qpkwik(const real32_T Linv_data[], const int32_T Linv_size[2], const
                   ...
                   real32_T f[4], int16_T n, real32_T x_data[], int32_T *x_size,
                   real32_T lambda[5], real32_T *status, int16_T iA[5], DW *rtDW);
static void mpcActiveSetSolver(const real32_T H_data[], const int32_T H_size[2],
  const real32_T f[4], real32_T x_data[], int32_T *x_size, real32_T *exitflag,
  DW *rtDW);
  ...
  rtu_f[4], boolean_T *rty_unsuited)
{
  ...
  /*  pre calculations */
  /* MATLAB Function 'impl_modul_mpc_single/detect_unsuited_sequence_1': '<S3>:1' */
  ...
  DW_formulate_qp_1 *localDW)
{
  ...
  /* MATLAB Function 'impl_modul_mpc_single/formulate_qp_1': '<S9>:1' */
  ...
  DW_state_space_mdl_1 *localDW)
{
  ...
  /* MATLAB Function 'impl_modul_mpc_single/state_space_mdl_1': '<S18>:1' */
                     ...
                     B_data[], const int32_T B_size[2], DW *rtDW)
{
  ...
  if ((B_size[1] != 0) && ((B_size[0] != 0) && (B_size[1] != 0))) {
    ...
    while (rtDW->j_h <= rtDW->nB) {
      ...
      rtDW->k = 1;
      while (rtDW->k - 1 <= rtDW->n_l - 1) {
        rtDW->kAcol = (rtDW->k - 1) * A_size[0] - 1;
        rtDW->i4 = rtDW->k + rtDW->jBcol;
        if (B_data[rtDW->i4] != 0.0F) {
          B_data[rtDW->i4] /= A_data[rtDW->k + rtDW->kAcol];
          rtDW->i_bn = rtDW->k + 1;
          while (rtDW->i_bn <= rtDW->n_l) {
            tmp = rtDW->i_bn + rtDW->jBcol;
            B_data[tmp] -= B_data[rtDW->i4] * A_data[rtDW->i_bn + rtDW->kAcol];
            rtDW->i_bn++;
          }
        }
...
{
  rtDW->i_j = 1;
  while (rtDW->i_j - 1 <= n - 1) {
    x_data[(int16_T)rtDW->i_j - 1] = ((-Hinv_data[(int16_T)rtDW->i_j - 1] * f[0]
      + -Hinv_data[((int16_T)rtDW->i_j + Hinv_size[0]) - 1] * f[1]) +
      -Hinv_data[((int16_T)rtDW->i_j + (Hinv_size[0] << 1)) - 1] * f[2]) +
      -Hinv_data[((int16_T)rtDW->i_j + Hinv_size[0] * 3) - 1] * f[3];
    rtDW->i_j++;
  }
}
...
static real32_T norm(const real32_T x_data[], const int32_T *x_size, DW *rtDW)
{
  ...
  if (*x_size == 1) {
    ...
    while (rtDW->k_f <= *x_size - 1) {
      rtDW->absxk_p = fabsf(x_data[rtDW->k_f]);
...
static real32_T xnrm2(int32_T n, const real32_T x_data[], int32_T ix0, DW *rtDW)
{
  ...
  if (n >= 1) {
    if (n == 1) {
      ...
      rtDW->kend = (ix0 + n) - 1;
      rtDW->k_d = ix0;
      while (rtDW->k_d <= rtDW->kend) {
        rtDW->absxk = fabsf(x_data[rtDW->k_d - 1]);
    ...
    while (rtDW->i_l <= rtDW->minmn - 1) {
      ...
      if (rtDW->i_l + 1 < rtDW->m_p) {
        ...
        if (rtDW->mmi > 0) {
          ...
          if (rtDW->beta1 != 0.0F) {
            rtDW->beta1 = rt_hypotf_o(A_data[rtDW->ii], rtDW->beta1, rtDW);
            ...
            if (fabsf(rtDW->beta1) < 9.86076132E-32F) {
              ...
              rtDW->beta1 = rt_hypotf_o(rtDW->b_atmp, xnrm2(rtDW->mmi - 1,
                   ...
                   real32_T B_0[4], real32_T C_data[], int32_T *C_size, DW *rtDW)
{
  ...
  *C_size = A_size[0];
  rtDW->i_bj = 0;
  while (rtDW->i_bj <= rtDW->m_e) {
    C_data[rtDW->i_bj] = 0.0F;
    rtDW->i_bj++;
  }

  rtDW->i_bj = 0;
  while (rtDW->i_bj <= A_size[1] - 1) {
    rtDW->aoffset = rtDW->i_bj * A_size[0];
    rtDW->b_i_j = 0;
    while (rtDW->b_i_j <= rtDW->m_e) {
      C_data[rtDW->b_i_j] += A_data[rtDW->aoffset + rtDW->b_i_j] * B_0
        [rtDW->i_bj];
      rtDW->b_i_j++;
    }
...
static void ResetToColdStart(int16_T iA[5], int16_T iC[5], DW *rtDW)
{
  for (rtDW->i_a = 0; rtDW->i_a < 5; rtDW->i_a++) {
    iA[rtDW->i_a] = 0;
    iC[rtDW->i_a] = 0;
  }
                   ...
                   real32_T lambda[5], real32_T *status, int16_T iA[5], DW *rtDW)
{
  ...
  *x_size = n;
  ...
  if (rtDW->nA > 0) {
    ...
    do {
      exitg3 = 0;
      if ((!DualFeasible) && (rtDW->nA > 0) && (*status <= 200.0F)) {
        ...
        if (rtDW->Xnorm0 < 0.0F) {
          ...
          while (rtDW->i_g - 1 <= rtDW->nA - 1) {
            ...
            while (rtDW->kNext <= rtDW->nA) {
              ...
              rtDW->H_g1 = 1;
              while (rtDW->H_g1 - 1 <= rtDW->nA - 1) {
                rtDW->loop_ub_m = rtDW->RLinv_size[0] * ((int16_T)rtDW->H_g1 - 1);
          ...
          while (rtDW->i_g - 1 <= n - 1) {
            ...
            rtDW->H_g1 = 1;
            while (rtDW->H_g1 - 1 <= rtDW->nA - 1) {
              rtDW->lambda_tmp_tmp = n + (int16_T)rtDW->H_g1;
              if (rtDW->lambda_tmp_tmp > 32767) {
                rtDW->lambda_tmp_tmp = 32767;
              } else if (rtDW->lambda_tmp_tmp < -32768) {
                rtDW->lambda_tmp_tmp = -32768;
              ...
              rtDW->Opt_data[(int16_T)rtDW->i_g - 1] += rtDW->D_data[((int16_T)
                rtDW->i_g + rtDW->D_size[0] * ((int16_T)rtDW->H_g1 - 1)) - 1] *
          ...
          while (rtDW->i_g - 1 <= rtDW->nA - 1) {
            ...
            while (rtDW->H_g1 - 1 <= rtDW->nA - 1) {
              ...
              rtDW->Opt_data[rtDW->loop_ub_m - 1] = rtDW->U_data[((int16_T)
                rtDW->i_g + rtDW->U_size[0] * ((int16_T)rtDW->H_g1 - 1)) - 1] *
                rtDW->Rhs_data[rtDW->i3 - 1] + rtDW->Opt_data[rtDW->i2 - 1];
              rtDW->H_g1++;
            }
          ...
          if (rtDW->kDrop <= 0) {
            ...
            *x_size = n;
            if (0 <= rtDW->loop_ub_m - 1) {
              memcpy(&x_data[0], &rtDW->Opt_data[0], rtDW->loop_ub_m * sizeof
                     (real32_T));
            }
          } else {
            (*status)++;
  ...
  if (guard1) {
    ...
    while ((!exitg2) && (*status <= 200.0F)) {
      ...
      if (rtDW->kNext <= 0) {
        exitg2 = true;
      } else if (*status == 200.0F) {
        *status = 0.0F;
        exitg2 = true;
      } else {
        do {
          exitg1 = 0;
          if ((rtDW->kNext > 0) && (*status <= 200.0F)) {
  ...
  DW *rtDW)
{
  ...
  while (rtDW->j <= H_size[1] - 1) {
    ...
    rtDW->i_f = 0;
    while (rtDW->i_f <= rtDW->m - 1) {
      rtDW->Hinv_data[rtDW->coffset + rtDW->i_f] = 0.0F;
      rtDW->i_f++;
    }

    rtDW->i_f = 0;
    while (rtDW->i_f <= H_size[0] - 1) {
      rtDW->bkj = H_data[rtDW->boffset + rtDW->i_f];
      rtDW->b_i = 0;
      while (rtDW->b_i <= rtDW->m - 1) {
        rtDW->i1 = rtDW->coffset + rtDW->b_i;
        rtDW->Hinv_data[rtDW->i1] += H_data[rtDW->b_i * H_size[0] + rtDW->i_f] *
          rtDW->bkj;
        rtDW->b_i++;
      }
...
/* Model step function */
void impl_modul_mpc_single_step(RT_MODEL *const rtM)
{
  DW *rtDW = rtM->dwork;
  ExtU *rtU = (ExtU *) rtM->inputs;
  ExtY *rtY = (ExtY *) rtM->outputs;
  boolean_T rtb_unsuited;
  boolean_T rtb_unsuited_b;
  boolean_T rtb_unsuited_be;
  boolean_T rtb_unsuited_h;
  boolean_T rtb_unsuited_k;
  boolean_T rtb_unsuited_n;
   ...
   */
  /* MATLAB Function 'impl_modul_mpc_single/seq_2_act_sw_all': '<S17>:1' */
  ...
  /* MATLAB Function 'impl_modul_mpc_single/act_sw_2_dq_volts_all': '<S2>:1' */
  ...
  formulate_qp_1(rtDW->m3, rtDW->m2, rtDW->m1, rtDW->m0, rtU->i_dq_pu[0],
                 ...
                 rtDW->H_g, rtDW->f_n, rtDW->M_i, rtDW->r_k,
  ...
  formulate_qp_1(rtDW->m3, rtDW->m2, rtDW->m1, rtDW->m0, rtU->i_dq_pu[0],
                 rtU->i_dq_pu[1], rtU->i_d_ref_pu, rtU->i_q_ref_pu, rtU->lambda,
                 rtDW->H_o, rtDW->f_k, rtDW->M_lo, rtDW->r_b,
  ...
  formulate_qp_1(rtDW->m3, rtDW->m2, rtDW->m1, rtDW->m0, rtU->i_dq_pu[0],
                 rtU->i_dq_pu[1], rtU->i_d_ref_pu, rtU->i_q_ref_pu, rtU->lambda,
                 rtDW->H_n, rtDW->f_a, rtDW->M_f, rtDW->r_l,
  ...
  formulate_qp_1(rtDW->m3, rtDW->m2, rtDW->m1, rtDW->m0, rtU->i_dq_pu[0],
                 rtU->i_dq_pu[1], rtU->i_d_ref_pu, rtU->i_q_ref_pu, rtU->lambda,
                 rtDW->H_hd, rtDW->f_c, rtDW->M_l, rtDW->r_d,
  ...
  formulate_qp_1(rtDW->m3, rtDW->m2, rtDW->m1, rtDW->m0, rtU->i_dq_pu[0],
                 rtU->i_dq_pu[1], rtU->i_d_ref_pu, rtU->i_q_ref_pu, rtU->lambda,
                 rtDW->H_h, rtDW->f_m, rtDW->M_p, rtDW->r_f,
  ...
  detect_unsuited_sequence_1(rtDW->H_g, rtDW->f_n, &rtb_unsuited_b);

  /* MATLAB Function: '<S1>/detect_unsuited_sequence_2' */
  detect_unsuited_sequence_1(rtDW->H_o, rtDW->f_k, &rtb_unsuited_k);

  /* MATLAB Function: '<S1>/detect_unsuited_sequence_3' */
  detect_unsuited_sequence_1(rtDW->H_n, rtDW->f_a, &rtb_unsuited_n);

  /* MATLAB Function: '<S1>/detect_unsuited_sequence_4' */
  detect_unsuited_sequence_1(rtDW->H_hd, rtDW->f_c, &rtb_unsuited_be);

  /* MATLAB Function: '<S1>/detect_unsuited_sequence_5' */
  detect_unsuited_sequence_1(rtDW->H_h, rtDW->f_m, &rtb_unsuited_h);
   ...
   */
  rtDW->tmpForInput_tmp[0] = rtb_unsuited_b;
  rtDW->tmpForInput_tmp[1] = rtb_unsuited_k;
  rtDW->tmpForInput_tmp[2] = rtb_unsuited_n;
  rtDW->tmpForInput_tmp[3] = rtb_unsuited_be;
  rtDW->tmpForInput_tmp[4] = rtb_unsuited_h;
  rtDW->tmpForInput_tmp[5] = rtb_unsuited;
  rtDW->i = 0;
  rtDW->u = 0U;
  for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 6; rtDW->x_tmp_tmp++) {
    rtDW->u += rtDW->tmpForInput_tmp[rtDW->x_tmp_tmp];
  }

  /* MATLAB Function: '<S1>/qp_solver_for_suited_seq_AS' */
  /* MATLAB Function 'impl_modul_mpc_single/qp_solver_for_suited_seq_AS': '<S16>:1' */
  ...
  if (((uint8_T)rtDW->u == 4) || ((uint8_T)rtDW->u == 5)) {
    ...
    if (!rtb_unsuited_b) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      linsolve(rtDW->H_g, rtDW->B_size, rtDW->b_I_data, rtDW->b_I_size,
               rtDW->Linv_data, rtDW->Linv_size, rtDW);

      /*  solve QP */
      /*      [x_tmp, exitflag] = mpcActiveSetSolver(double(Linv),double(-f),double(A),double(b),double(Aeq),double(beq),iA0,options); */
      /* '<S16>:1:84' [x_tmp, exitflag] = mpcActiveSetSolver(Linv,-f,A,b,Aeq,beq,iA0,options); */
      rtDW->x1[0] = -rtDW->f_n[0];
      rtDW->x1[1] = -rtDW->f_n[1];
      rtDW->x1[2] = -rtDW->f_n[2];
      rtDW->x1[3] = -rtDW->f_n[3];

      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->x1,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
                         &rtY->iterations_qp[0], rtDW);

      /* '<S16>:1:85' x = single(x_tmp(1:4,1)); */
      /* '<S16>:1:86' iter = single(exitflag); */
      /* '<S16>:1:43' J1 = x1'*(M1'*M1)*x1-2*r1'*M1*x1; */
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x1[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H_g[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H_g[rtDW->loop_ub] += rtDW->M_i[(rtDW->i << 3) + rtDW->d] *
              rtDW->M_i[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      rtDW->f_kk = 0.0F;
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H_g[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data[3]
                             + (rtDW->H_g[rtDW->x_tmp_tmp + 2] *
          rtDW->x_tmp_data[2] + (rtDW->H_g[rtDW->x_tmp_tmp + 1] *
          rtDW->x_tmp_data[1] + rtDW->H_g[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0])))
          * rtDW->x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M_i[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r_k[rtDW->x_tmp_tmp]);
        }

        rtDW->f_kk += rtDW->f_n[rtDW->i] * rtDW->x_tmp_data[rtDW->i];
      }

      rtDW->SinCos1_o1 = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
    ...
    if (!rtb_unsuited_k) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      memcpy(&rtDW->H_g[0], &rtDW->H_o[0], sizeof(real32_T) << 4U);
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      /*      [x_tmp, exitflag] = mpcActiveSetSolver(double(Linv),double(-f),double(A),double(b),double(Aeq),double(beq),iA0,options); */
      /* '<S16>:1:84' [x_tmp, exitflag] = mpcActiveSetSolver(Linv,-f,A,b,Aeq,beq,iA0,options); */
      rtDW->x2[0] = -rtDW->f_k[0];
      rtDW->x2[1] = -rtDW->f_k[1];
      rtDW->x2[2] = -rtDW->f_k[2];
      rtDW->x2[3] = -rtDW->f_k[3];

      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->x2,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
                         &rtY->iterations_qp[1], rtDW);

      /* '<S16>:1:85' x = single(x_tmp(1:4,1)); */
      /* '<S16>:1:86' iter = single(exitflag); */
      /* '<S16>:1:47' J2 = x2'*(M2'*M2)*x2-2*r2'*M2*x2; */
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x2[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H_o[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H_o[rtDW->loop_ub] += rtDW->M_lo[(rtDW->i << 3) + rtDW->d] *
              rtDW->M_lo[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H_o[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data[3]
                             + (rtDW->H_o[rtDW->x_tmp_tmp + 2] *
          rtDW->x_tmp_data[2] + (rtDW->H_o[rtDW->x_tmp_tmp + 1] *
          rtDW->x_tmp_data[1] + rtDW->H_o[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0])))
          * rtDW->x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M_lo[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r_b[rtDW->x_tmp_tmp]);
        }
      ...
      rtDW->J_1to6[1] = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
    ...
    if (!rtb_unsuited_n) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      memcpy(&rtDW->H_g[0], &rtDW->H_n[0], sizeof(real32_T) << 4U);
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      /*      [x_tmp, exitflag] = mpcActiveSetSolver(double(Linv),double(-f),double(A),double(b),double(Aeq),double(beq),iA0,options); */
      /* '<S16>:1:84' [x_tmp, exitflag] = mpcActiveSetSolver(Linv,-f,A,b,Aeq,beq,iA0,options); */
      rtDW->x3[0] = -rtDW->f_a[0];
      rtDW->x3[1] = -rtDW->f_a[1];
      rtDW->x3[2] = -rtDW->f_a[2];
      rtDW->x3[3] = -rtDW->f_a[3];

      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->x3,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
                         &rtY->iterations_qp[2], rtDW);

      /* '<S16>:1:85' x = single(x_tmp(1:4,1)); */
      /* '<S16>:1:86' iter = single(exitflag); */
      /* '<S16>:1:51' J3 = x3'*(M3'*M3)*x3-2*r3'*M3*x3; */
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x3[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H_n[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H_n[rtDW->loop_ub] += rtDW->M_f[(rtDW->i << 3) + rtDW->d] *
              rtDW->M_f[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H_n[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data[3]
                             + (rtDW->H_n[rtDW->x_tmp_tmp + 2] *
          rtDW->x_tmp_data[2] + (rtDW->H_n[rtDW->x_tmp_tmp + 1] *
          rtDW->x_tmp_data[1] + rtDW->H_n[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0])))
          * rtDW->x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M_f[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r_l[rtDW->x_tmp_tmp]);
        }
      ...
      /* MATLAB Function: '<S1>/min_Costs' */
      rtDW->J_1to6[2] = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
    ...
    if (!rtb_unsuited_be) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      memcpy(&rtDW->H_g[0], &rtDW->H_hd[0], sizeof(real32_T) << 4U);
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      /*      [x_tmp, exitflag] = mpcActiveSetSolver(double(Linv),double(-f),double(A),double(b),double(Aeq),double(beq),iA0,options); */
      /* '<S16>:1:84' [x_tmp, exitflag] = mpcActiveSetSolver(Linv,-f,A,b,Aeq,beq,iA0,options); */
      rtDW->x4[0] = -rtDW->f_c[0];
      rtDW->x4[1] = -rtDW->f_c[1];
      rtDW->x4[2] = -rtDW->f_c[2];
      rtDW->x4[3] = -rtDW->f_c[3];

      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->x4,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
                         &rtY->iterations_qp[3], rtDW);

      /* '<S16>:1:85' x = single(x_tmp(1:4,1)); */
      /* '<S16>:1:86' iter = single(exitflag); */
      /* '<S16>:1:55' J4 = x4'*(M4'*M4)*x4-2*r4'*M4*x4; */
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x4[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H_hd[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H_hd[rtDW->loop_ub] += rtDW->M_l[(rtDW->i << 3) + rtDW->d] *
              rtDW->M_l[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H_hd[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data
                             [3] + (rtDW->H_hd[rtDW->x_tmp_tmp + 2] *
          rtDW->x_tmp_data[2] + (rtDW->H_hd[rtDW->x_tmp_tmp + 1] *
          rtDW->x_tmp_data[1] + rtDW->H_hd[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0])))
          * rtDW->x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M_l[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r_d[rtDW->x_tmp_tmp]);
        }
      ...
      /* MATLAB Function: '<S1>/min_Costs' */
      rtDW->J_1to6[3] = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
    ...
    if (!rtb_unsuited_h) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      memcpy(&rtDW->H_g[0], &rtDW->H_h[0], sizeof(real32_T) << 4U);
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      /*      [x_tmp, exitflag] = mpcActiveSetSolver(double(Linv),double(-f),double(A),double(b),double(Aeq),double(beq),iA0,options); */
      /* '<S16>:1:84' [x_tmp, exitflag] = mpcActiveSetSolver(Linv,-f,A,b,Aeq,beq,iA0,options); */
      rtDW->x5[0] = -rtDW->f_m[0];
      rtDW->x5[1] = -rtDW->f_m[1];
      rtDW->x5[2] = -rtDW->f_m[2];
      rtDW->x5[3] = -rtDW->f_m[3];

      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->x5,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
                         &rtY->iterations_qp[4], rtDW);

      /* '<S16>:1:85' x = single(x_tmp(1:4,1)); */
      /* '<S16>:1:86' iter = single(exitflag); */
      /* '<S16>:1:59' J5 = x5'*(M5'*M5)*x5-2*r5'*M5*x5; */
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x5[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H_h[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H_h[rtDW->loop_ub] += rtDW->M_p[(rtDW->i << 3) + rtDW->d] *
              rtDW->M_p[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H_h[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data[3]
                             + (rtDW->H_h[rtDW->x_tmp_tmp + 2] *
          rtDW->x_tmp_data[2] + (rtDW->H_h[rtDW->x_tmp_tmp + 1] *
          rtDW->x_tmp_data[1] + rtDW->H_h[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0])))
          * rtDW->x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M_p[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r_f[rtDW->x_tmp_tmp]);
        }
      ...
      /* MATLAB Function: '<S1>/min_Costs' */
      rtDW->J_1to6[4] = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
    ...
    if (!rtb_unsuited) {
      ...
      rtDW->B_size[0] = 4;
      rtDW->B_size[1] = 4;
      memcpy(&rtDW->H_g[0], &rtDW->H[0], sizeof(real32_T) << 4U);
      cholesky(rtDW->H_g, rtDW->B_size, rtDW);
      ...
      /* Outport: '<Root>/iterations_qp' */
      mpcActiveSetSolver(rtDW->Linv_data, rtDW->Linv_size, rtDW->f_m,
                         rtDW->x_tmp_data, &rtDW->x_tmp_size,
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x6[rtDW->i] = rtDW->x_tmp_data[rtDW->i];
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 4; rtDW->x_tmp_tmp++) {
          rtDW->loop_ub = rtDW->i + (rtDW->x_tmp_tmp << 2);
          rtDW->H[rtDW->loop_ub] = 0.0F;
          for (rtDW->d = 0; rtDW->d < 8; rtDW->d++) {
            rtDW->H[rtDW->loop_ub] += rtDW->M[(rtDW->i << 3) + rtDW->d] *
              rtDW->M[(rtDW->x_tmp_tmp << 3) + rtDW->d];
          }
        }
      }
      ...
      for (rtDW->i = 0; rtDW->i < 4; rtDW->i++) {
        rtDW->x_tmp_tmp = rtDW->i << 2;
        rtDW->SinCos1_o2 += (rtDW->H[rtDW->x_tmp_tmp + 3] * rtDW->x_tmp_data[3]
                             + (rtDW->H[rtDW->x_tmp_tmp + 2] * rtDW->x_tmp_data
          [2] + (rtDW->H[rtDW->x_tmp_tmp + 1] * rtDW->x_tmp_data[1] + rtDW->
                 H[rtDW->x_tmp_tmp] * rtDW->x_tmp_data[0]))) * rtDW->
          x_tmp_data[rtDW->i];
        rtDW->f_n[rtDW->i] = 0.0F;
        for (rtDW->x_tmp_tmp = 0; rtDW->x_tmp_tmp < 8; rtDW->x_tmp_tmp++) {
          rtDW->f_n[rtDW->i] += rtDW->M[(rtDW->i << 3) + rtDW->x_tmp_tmp] *
            (2.0F * rtDW->r[rtDW->x_tmp_tmp]);
        }
      ...
      /* MATLAB Function: '<S1>/min_Costs' */
      rtDW->J_1to6[5] = rtDW->SinCos1_o2 - rtDW->f_kk;
    }
  }
   ...
   */
  /* MATLAB Function 'impl_modul_mpc_single/min_Costs': '<S15>:1' */
  ...
  /* MATLAB Function 'impl_modul_mpc_single/tz': '<S24>:1' */
  ...
  for (rtDW->i = 0; rtDW->i < 3; rtDW->i++) {
     ...
     *  MATLAB Function: '<S1>/tz_2_tph'
     */
    rtDW->x_tmp_tmp = (rtDW->d - 1) << 2;
    rtDW->tz[rtDW->i] = rtDW->rtb_x1_m[rtDW->x_tmp_tmp + 3] * (real32_T)a
      [rtDW->i + 9] + (rtDW->rtb_x1_m[rtDW->x_tmp_tmp + 2] * (real32_T)a[rtDW->i
                       + 6] + (rtDW->rtb_x1_m[rtDW->x_tmp_tmp + 1] * (real32_T)
      a[rtDW->i + 3] + rtDW->rtb_x1_m[rtDW->x_tmp_tmp] * (real32_T)a[rtDW->i]));
  }
   ...
   *  MATLAB Function: '<S1>/tz_2_tph'
   */
  /* MATLAB Function 'impl_modul_mpc_single/tz_2_tph': '<S25>:1' */
  ...
  rtY->unsuited_qp[0] = rtb_unsuited_b;
  rtY->unsuited_qp[1] = rtb_unsuited_k;
  rtY->unsuited_qp[2] = rtb_unsuited_n;
  rtY->unsuited_qp[3] = rtb_unsuited_be;
  rtY->unsuited_qp[4] = rtb_unsuited_h;
  rtY->unsuited_qp[5] = rtb_unsuited;
}

/* Model initialize function */
void impl_modul_mpc_single_initialize(RT_MODEL *const rtM)
{
  ExtU *rtU = (ExtU *) rtM->inputs;
  ExtY *rtY = (ExtY *) rtM->outputs;
