 ...
 *
 * deterministic_observer_active
 *         — true when the deterministic observer ran in this ISR step.
 *
 * det_*   — diagnostics from the deterministic observer when
 *           deterministic_observer_active is true. det_omega_s_rad_s remains
 *           available as a separate resonant-controller frequency reference.
 *
 * kf_*    — KF diagnostics (zero-initialized when kf_ready is false).
 */
typedef struct {
    im_rotor_flux_observer_output_t output;
    bool deterministic_observer_active;
 ...
 *
 * The deterministic observer runs when use_deterministic_observer is true or
 * when it is needed as a safe fallback because the KF is not ready or not
 * selected. The KF observer runs only when kf_ready is true.
 *
 ...
 * @param use_deterministic_observer
 *                           True to always run the deterministic observer.
 *                           False bypasses it when the KF is selected and ready.
