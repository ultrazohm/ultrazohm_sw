...
void XUz_pi_controller_sample_Set_output_r(XUz_pi_controller_sample *InstancePtr, u64 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XUz_pi_controller_sample_WriteReg(InstancePtr->Control_BaseAddress, XUZ_PI_CONTROLLER_SAMPLE_CONTROL_ADDR_OUTPUT_R_DATA, (u32)(Data));
    XUz_pi_controller_sample_WriteReg(InstancePtr->Control_BaseAddress, XUZ_PI_CONTROLLER_SAMPLE_CONTROL_ADDR_OUTPUT_R_DATA + 4, (u32)(Data >> 32));
}

u64 XUz_pi_controller_sample_Get_output_r(XUz_pi_controller_sample *InstancePtr) {
    u64 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XUz_pi_controller_sample_ReadReg(InstancePtr->Control_BaseAddress, XUZ_PI_CONTROLLER_SAMPLE_CONTROL_ADDR_OUTPUT_R_DATA);
    Data += (u64)XUz_pi_controller_sample_ReadReg(InstancePtr->Control_BaseAddress, XUZ_PI_CONTROLLER_SAMPLE_CONTROL_ADDR_OUTPUT_R_DATA + 4) << 32;
    return Data;
}
