...
First, Global value for HLS Ip is defined and its initialized with device status through the function ``init_pi_controller()``. An array of float ``mem`` is declared to allocate the memory for the result of the operation.
A pointer variable ``result`` is declared and is pointed to the desired memory location of DDR Memory ``float *result = mem;``. First we set the output_r to the address location ``mem``. Then we call the ``XUz_pi_controller_sample_Start`` and we wait till the operation is done.
...
------------
   ...
   - Cache lines has to be invalidated before writing the value into the memory.
      ...
      This is done using the command ``Xil_DCacheFlushRange((u64)result, (sizeof(mem)*NO_OF_ELEMENTS));`` in our code.
