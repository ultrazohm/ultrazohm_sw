...
void nn_init(void)
{
    ...
    Global_Data.objects.matrix_input = uz_matrix_init(&input_matrix, x_9, UZ_MATRIX_SIZE(x_9), 1U, NUMBER_OF_INPUTS_9N);
    Global_Data.objects.nn_layer = uz_nn_init(config_9nn, 4U);

    struct uz_mlp_three_layer_ip_config_t config = {
        .base_address = XPAR_UZ_USER_UZ_MLP_THREE_LAYER_0_BASEADDR,
        .use_axi_input = true,
        .software_network = Global_Data.objects.nn_layer};
