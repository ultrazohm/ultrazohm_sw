...
#define DQN_PT1_EVAL_LOG_PATH "../../../workshop/logs/ceedling_eval_log_module.csv"
...
// The agent returns the action value, not its index. The value comes straight
// out of the same table, so an exact match recovers the index for the log.
static uint32_t dqn_pt1_action_index_of(float action_value)
{
    for (uint32_t i = 0U; i < DQN_PT1_NUMBER_OF_OUTPUTS; i++)
    {
        if (dqn_pt1_action_values[i] == action_value)
        {
            return i;
        }
    }
    TEST_FAIL_MESSAGE("agent returned an action outside the configured action table");
    return 0U;
}
...
{
    fprintf(file,
            "%lu;%lu;%lu;%.9g;%.9g;%.9g;%lu;%.9g;%.9g;%.9g;%.9g;%.9g;%.9g\n",
            (unsigned long)time,
...
void test_uz_dqn_pt1_eval_module_export_to_csv(void)
{
    struct uz_dqn_agent_config config = {
        .layers = dqn_pt1_layer_config,
        .number_of_layers = DQN_PT1_NUMBER_OF_LAYERS,
        .action_values = dqn_pt1_action_values,
        .number_of_actions = DQN_PT1_NUMBER_OF_OUTPUTS,
    };
    ...
    for (uint32_t episode_index = 0U; episode_index < DQN_PT1_EVAL_EPISODES; episode_index++)
    {
        ...
        {
            const float action_value = uz_dqn_agent_step(agent, y, reference);
            const uint32_t action_index = dqn_pt1_action_index_of(action_value);
