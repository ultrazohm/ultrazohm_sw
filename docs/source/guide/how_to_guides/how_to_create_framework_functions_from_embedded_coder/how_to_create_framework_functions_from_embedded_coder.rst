...
C code that is generated by using the :ref:`embedded_coder` is not directly compatible with the software guidelines and requires a wrapper module to be included in the software framework.
...
The following guide builds on the example model of :ref:`embedded_coder` and highlights how to create an interface that is consistent with the :ref:`software_development_guidelines` (multiple-instance module).
...
1. Open ``ultrazohm_sw`` with VS Code and the remote container (see :ref:`vscode_remote_container`)
