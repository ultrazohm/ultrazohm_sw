...
static TmrCntrSetup SettingsTable[NUM_TTC_PS_DEVICES] = {
    ...
    [TTC_TIMESTAMP_DEVICE_ID]{
        ...
        .Prescaler = 6, // div_factor = 2 ^ (prescaler + 1) = (2 << prescaler)
...
static int bsp_timer_init_irq(uint32_t ttc_device_id, uint32_t ttc_tick_intr_id)
{
    ...
    XTtcPs_SetStatusHandler(&(TtcPsInst[ttc_device_id]),
    						&(TtcPsInst[ttc_device_id]),
...
void bsp_timer_init(void)
{
    ...
    // Manually disable TTC9
    // TODO Why is TTC9 running?! (FreeRTOS uses TTC0, no one else uses TTCs)
    *(uint32_t *)(XPAR_PSU_TTC_9_BASEADDR + XTTCPS_CNT_CNTRL_OFFSET) |= 0x01;
...
uint64_t bsp_timer_timestamp_u64_get(void)
{
	uint64_t timestamp;
	XTime_GetTime((XTime *)&timestamp);
	return timestamp;
}
