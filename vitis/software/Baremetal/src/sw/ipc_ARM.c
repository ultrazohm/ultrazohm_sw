...
void ipc_Control_func(uint32_t msgId, float value, DS_Data *data)
{
	...
	if (msgId != 0)
	{
        ...
        case (ParaID_EID_Admit_Params):
            //If FOC is used
            uz_CurrentControl_set_PMSM_parameters(data->objects.CC_instance, ParaID_Data.ElectricalID_Output->PMSM_parameters);
            uz_SetPoint_set_PMSM_config(data->objects.SP_instance, ParaID_Data.ElectricalID_Output->PMSM_parameters);
            break;
