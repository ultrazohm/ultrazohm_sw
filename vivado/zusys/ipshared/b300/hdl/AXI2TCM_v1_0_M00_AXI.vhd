	    ...
	    if (rising_edge (M_AXI_ACLK)) then                   
	      if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                     
	        ...
	        if (M_AXI_RVALID = '1' and axi_rready = '1' and M_AXI_RLAST = '0') then
	          expected_rdata <= DATA( (to_integer(unsigned(read_index))+2)*C_M_AXI_DATA_WIDTH-1 downto (to_integer(unsigned(read_index))+1)*C_M_AXI_DATA_WIDTH); 
			  -- use read_index to index DATA_IN in chuncks of 32bit 
	        end if;                                          
	      end if;                                            
	    end if;                                              
	  end process;                                           
