...
uz_pmsm_linear_t* uz_pmsm_linear_init() {
    uz_pmsm_linear_t* self = uz_pmsm_linear_allocation();
    return (self);
}

void uz_pmsm_linear_step(uz_pmsm_linear_t *self, uz_3ph_dq_t v, float omega_el)
{
    uz_assert_not_NULL(self);
    uz_assert(self->is_ready);
    uz_3ph_dq_t i_k = {0.0f};
    i_k.d = (self->phi_k.d - self->psi_pm) / self->L_q;
    i_k.q = (self->phi_k.d) / self->L_d;
    ...
    self->i.d = (self->phi_k.d - self->psi_pm) / self->L_q;
    self->i.q = (self->phi_k.d) / self->L_d;
