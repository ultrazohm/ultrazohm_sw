...
.. warning::
   ...
   Breaking change: ``DIGS3C_Shared_ReqSafeState`` is intentionally no longer a static safe-state level.
   The signal is now a heartbeat because the D-Slot CPLD's 1.8 V I/O supply can ramp down faster than the CPLD core supply.
   During that interval the CPLD can be outside its specified operating conditions, and a level-based interpretation can no longer guarantee that the system is in normal operation.
   Normal operation is therefore encoded as an actively generated heartbeat.
   If the heartbeat stops or becomes invalid, every updated D-Slot CPLD enters its safe state.

**Heartbeat based D-Slot safe-state request**

The MachXO2 S3C bitstream now drives ``DIGS3C_Shared_ReqSafeState`` as a heartbeat instead of a static level.
The S3C finite-state machine still creates the internal safe-state request, but the output pin is generated by the shared ``sXc_heartbeat_sender`` component from ``xo2_libraries/lib_sXc.vhd``.
When no safe state is requested, the sender toggles ``DIGS3C_Shared_ReqSafeState`` with an approximately 50 kHz heartbeat.
When the S3C requests a safe state, the heartbeat stops and the line is held at its inactive level.
...
Every MachXO2 D-Slot CPLD receives this signal on its ``reqsafestate`` input and validates it with the common ``dslot_heartbeat_receiver`` component.
If the heartbeat is missing, too slow, too fast or otherwise invalid, the D-Slot CPLD asserts its local safe-state request and disables output forwarding.
This makes the default safe-state behavior fail-safe for a stuck wire, a stopped S3C clock, a wrong static level or an S3C-requested stop.

The S3C still forwards the per-slot output-enable requests conditionally through ``DIGS3C_SlotD_SlotOE``.
The heartbeat only changes the interpretation of ``DIGS3C_Shared_ReqSafeState``; it does not replace the SlotOE request path.
Both S3C and D-Slot MachXO2 projects are built with Synplify Pro and VHDL-2008 enabled, because the shared ``xo2_libraries/lib_sXc.vhd`` components rely on VHDL-2008 generics and interfaces.
