...
.. code-block:: c
  ...
  int main(void) {
     ...
     struct uz_dq_t output = uz_SpeedControl_sample(instance, omega_el_rad_per_sec, n_ref_rpm, id_ref_Ampere, ext_clamping);
  }
...
.. tikz:: PI-Controller
  ...
  \node[font=\footnotesize] (SC_ref) at ($(FOC_speed.west)+(0.9,0.2)$) {n\_ref\_rpm};
  ...
  \node[font=\footnotesize] (SC_actual) at ($(FOC_speed.west)+(1.8,-0.4)$) {omega\_el\_rad\_per\_sec};
  \node[font=\footnotesize] (SC_id) at ($(FOC_speed.west)+(1.3,-1.2)$) {id\_ref\_Ampere};
  ...
  \node[font=\footnotesize] at ($(n_PIController.west)+(0.96,-0.9)$) {actualValue};
  \node[font=\footnotesize] at ($(n_PIController.west)+(1.16,-0.3)$) {referenceValue};
  ...
  \draw[-latex](SC_ref.east) -- ($(n_PIController.west)+(0,-0.3)$);
  \draw[-latex](SC_actual.east) -- ($(n_PIController.west)+(0,-0.9)$);
