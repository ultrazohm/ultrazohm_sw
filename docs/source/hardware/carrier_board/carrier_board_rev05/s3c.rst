...
The S3C drives the D-slot handshake with two independent shared signals:

.. list-table:: S3C shared D-slot signals
   :header-rows: 1
   :widths: 25 20 55
   ...
   * - Signal
     - Behaviour
     - Meaning
   * - ``DIGS3C_Shared_CarrierReady``
     - Heartbeat
     - Dynamic heartbeat output from the S3C to the shared D-slot signal. D-slot output programs validate this signal before enabling output forwarding.
   * - ``DIGS3C_Shared_ReqSafeState``
     - Static ``0`` or ``1``
     - Static safe-state request. This signal does not toggle and is not interpreted as heartbeat.

``CarrierReady`` heartbeat
~~~~~~~~~~~~~~~~~~~~~~~~~~

The raw heartbeat is generated by the shared heartbeat sender and routed to ``DIGS3C_Shared_CarrierReady``.
Because the shared sender keeps its output statically high when disabled, the S3C top level masks the raw sender output with the heartbeat-enable condition.
Therefore the physical ``CarrierReady`` pin is low whenever the heartbeat is disabled.
...
``ReqSafeState`` static request
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``DIGS3C_Shared_ReqSafeState`` is driven statically by the S3C FSM.
It is independent of the heartbeat generation.

.. code-block:: vhdl

   DIGS3C_Shared_ReqSafeState <= dslot_reqsafe_static;

For D-slot programs that use the ``CarrierReady`` heartbeat, the meaning is:

* ``ReqSafeState = 0``: D-slot output forwarding may be allowed if the ``CarrierReady`` heartbeat is valid.
* ``ReqSafeState = 1``: D-slot output forwarding shall be disabled, even if the ``CarrierReady`` heartbeat is still valid.

S3C FSM behaviour
~~~~~~~~~~~~~~~~~

The ``CarrierReady`` heartbeat remains active while the carrier is powered and the S3C has not entered the explicit D-slot-down shutdown path.
It also remains active in ``Softerror`` and ``Harderror``.
In those states, outputs are disabled by the separate static ``ReqSafeState = 1`` signal, not by stopping the heartbeat.

``CarrierReady`` heartbeat is enabled in:

* ``Wait_State``
* ``EthernetPhy_Reset``
* ``Ack_previous_Harderror``
...
``CarrierReady`` heartbeat is low/disabled in:

* reset and initial/default states
* ``sleep_for_dslot_down``
* ``ppn6v = 0``
* debounced ``PG_Module`` feedback ``pg_som = 0``

.. figure:: img/s3c_dslot_heartbeat_sender.svg
   :width: 85%
   :align: center

   S3C sender side: ``CarrierReady`` carries the heartbeat; ``ReqSafeState`` remains static.

.. figure:: img/s3c_dslot_heartbeat_handshake.svg
   :width: 95%
   :align: center

   Heartbeat handshake between the S3C ``CarrierReady`` sender path and the D-Slot CPLD heartbeat receiver.

SlotOE passthrough
~~~~~~~~~~~~~~~~~~

The S3C side also contains a conditional pass-through that propagates the SlotOE request only when no system-wide force-disable is active:

.. code-block:: vhdl

   DIGS3C_SlotD_SlotOE <= DIGS3C_SlotD_ReqOE and (DIGS3C_SlotD_SlotOE'Range => NOT forceoutputdisable);

See :ref:`dslot_cpld_heartbeat_rev05` for the D-slot receiver behaviour.
**Default logic in the D-Slot bitstream**

The D-Slot CPLD feeds two status signals back to the S3C:

- ReqOE: D-Slot request for enabling the adapter-card output path; the S3C conditionally forwards it to SlotOE.
- SlotOK: asserted when output forwarding is allowed. For heartbeat-gated output implementations this requires a valid ``CarrierReady`` heartbeat, ``ReqSafeState = 0`` and all program-specific enable conditions.
