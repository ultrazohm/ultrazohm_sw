...
void uz_incrementalEncoder_hw_set_omegaPerOverSample(uint32_t base_address,float omega_per_over_sampl){
    ...
    uz_axi_write_int32(base_address+omega_AXI4_Data_Incremental_Encoder_v26,uz_convert_float_to_sfixed(omega_per_over_sampl,11));
}


void uz_incrementalEncoder_hw_set_timeout_value(uint32_t base_address,uint32_t timeout_ms){
	uz_assert_not_zero_uint32(base_address);
	uz_axi_write_uint32(base_address+timeout_value_AXI4_Data_Incremental_Encoder_v26, timeout_ms);	// direct Count-Value
}
