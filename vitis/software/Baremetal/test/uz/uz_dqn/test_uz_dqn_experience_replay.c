...
void test_uz_dqn_write_to_buffer(void)
{
    ...
    uz_dqn_experience_replay_t *buffertesting = uz_dqn_experience_replay_init(configbuffer,EXPERIENCE_BUFFER_LENGTH);
    uz_dqn_buffer_push_data(buffertesting,RewardData1,action1,obs,obs1);
    ObsData[0] = 1.0f;
    ObsData1[0] = 2.0f;
    float RewardData2 = -27.7f;
    float QData2 = 6.4f;
    uint32_t action2 = 0;
    uz_dqn_buffer_push_data(buffertesting,RewardData2,action2,obs,obs1);
    ObsData[1] = -2.0f;
    ObsData1[1] = -1.0f;
    float RewardData3 = 300.0f;
    float QData3 = 7.99f;
    uint32_t action3 = 3;
    uz_dqn_buffer_push_data(buffertesting,RewardData3,action3,obs,obs1);
...
void test_uz_dqn_overwrite_first_values(void){
    ...
    struct uz_matrix_t obs_matrix1 = {0};
    uz_matrix_t *obs1 = uz_matrix_init(&obs_matrix1, ObsData1, UZ_MATRIX_SIZE(ObsData1), 1, 5);
    uz_dqn_buffer_push_data(buffertesting,RewardData4,action4,obs,obs1);
