...
static void enter_atomic_PsiCalcState(ExtU_OnlineID_t *rtOnlineID_U, ExtY_OnlineID_t *rtOnlineID_Y, DW_OnlineID_t *rtOnlineID_DW) {
	...
	if (rtOnlineID_U->array_cleaned_flag) {
		...
		memcpy(&rtOnlineID_Y->OnlineID_output.psi_array[0], &rtOnlineID_U->psi_array_out[0], 600U * sizeof(real32_T));
	}
...
static void RefreshDataRegister(ExtU_OnlineID_t *rtOnlineID_U, ExtY_OnlineID_t *rtOnlineID_Y, DW_OnlineID_t *rtOnlineID_DW) {
	...
	if (rtOnlineID_DW->allow_meas && rtOnlineID_DW->omega_const && rtOnlineID_DW->iq_const && rtOnlineID_DW->id_const) {
		...
		/* Inport: '<Root>/OnlineIDConfig' */
		if (((!rtOnlineID_DW->LinPara_ident_outside) && (!rtOnlineID_U->OnlineIDConfig.allowPsiCalcOutside) && tmp && rtOnlineID_DW->omega_const && rtOnlineID_DW->iq_const)
		                || (rtOnlineID_U->OnlineIDConfig.allowPsiCalcOutside && tmp && rtOnlineID_DW->omega_const && rtOnlineID_DW->iq_const)) {
			/* Transition: '<S1>:378' */
			/* Transition: '<S1>:331' */
