...
struct uz_dq uz_dq_Transformation(struct uz_UVW input, float theta_el){
    ...
    output.d = (2.0f / 3.0f) * ( (cosf(theta_el) * input.U) + (cosf(theta_el -  ( (2.0f / 3.0f) * M_PI) ) * input.V) + (cosf(theta_el - ( (4.0f / 3.0f) * M_PI) ) * input.W) );
    output.q = (2.0f / 3.0f) * ( -1.0f * (sinf(theta_el) * input.U) - (sinf(theta_el -  ( (2.0f / 3.0f) * M_PI) ) * input.V) - (sinf(theta_el - ( (4.0f / 3.0f) * M_PI) ) * input.W) );
...
struct uz_UVW uz_inverse_dq_Transformation(struct uz_dq input, float theta_el){
    ...
    output.U = (cosf(theta_el) * input.d) - (sinf(theta_el) * input.q) + input.zero;
    output.V = (cosf(theta_el -  ( (2.0f / 3.0f) * M_PI) ) * input.d) - (sinf(theta_el - ( (2.0f / 3.0f) * M_PI) ) * input.q) + input.zero;
    output.W = (cosf(theta_el -  ( (4.0f / 3.0f ) * M_PI) ) * input.d) - (sinf(theta_el - ( (4.0f / 3.0f) * M_PI) ) * input.q) + input.zero;
    return(output);
}
