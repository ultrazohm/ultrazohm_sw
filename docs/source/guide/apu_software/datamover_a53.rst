...
.. mermaid::
    ...
    graph TB
      ...
      subgraph TCP["TCP worker: javascope_connection_thread"]
...
- ``javascope_connection_thread`` only starts building an Ethernet payload once at least one full batch is queued.
...
- ``javascope_connection_thread`` reads the JavaScope control connection with non-blocking ``lwip_recv`` and treats the incoming data as a TCP byte stream rather than as one message per ``recv`` call.
...
- ``javascope_server_thread`` and ``javascope_connection_thread`` enforce a single active JavaScope client at a time using atomic replace/release helpers around ``js_connection_established``.
