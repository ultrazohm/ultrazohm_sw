...
In the Vivado block-design the SI-Value-Output can be sliced with Slice-IP-Cores to get access to the SI-Values of the ADC-channels.
In the next figure the slice-blocks can be seen. Each of them slices one value from the vector.
The length of the sliced part is according to the used datatype 18 bit.
