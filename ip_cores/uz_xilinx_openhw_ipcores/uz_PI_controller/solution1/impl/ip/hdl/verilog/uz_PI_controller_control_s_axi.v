...
#(parameter
    ...
    output wire                          RVALID,
    input  wire                          RREADY,
    output wire [31:0]                   axi_referenceValue,
    output wire [31:0]                   axi_sampletime,
    output wire [31:0]                   axi_KI,
    output wire [31:0]                   axi_KP,
...
//------------------------Parameter----------------------
localparam
    ADDR_AXI_REFERENCEVALUE_DATA_0 = 6'h10,
    ADDR_AXI_REFERENCEVALUE_CTRL   = 6'h14,
    ADDR_AXI_SAMPLETIME_DATA_0     = 6'h18,
    ADDR_AXI_SAMPLETIME_CTRL       = 6'h1c,
    ADDR_AXI_KI_DATA_0             = 6'h20,
    ADDR_AXI_KI_CTRL               = 6'h24,
    ADDR_AXI_KP_DATA_0             = 6'h28,
    ADDR_AXI_KP_CTRL               = 6'h2c,
    ADDR_AXI_LIMIT_DATA_0          = 6'h30,
    ADDR_AXI_LIMIT_CTRL            = 6'h34,
    ADDR_AXI_RESET_DATA_0          = 6'h38,
    ADDR_AXI_RESET_CTRL            = 6'h3c,
    WRIDLE                         = 2'd0,
    WRDATA                         = 2'd1,
    WRRESP                         = 2'd2,
    WRRESET                        = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_axi_KI = 'b0;
    reg  [31:0]                   int_axi_KP = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_AXI_KI_DATA_0: begin
                    rdata <= int_axi_KI[31:0];
                end
                ADDR_AXI_KP_DATA_0: begin
                    rdata <= int_axi_KP[31:0];
                end
...
//------------------------Register logic-----------------
assign axi_referenceValue = int_axi_referenceValue;
assign axi_sampletime     = int_axi_sampletime;
assign axi_KI             = int_axi_KI;
assign axi_KP             = int_axi_KP;
assign axi_limit          = int_axi_limit;
assign axi_reset          = int_axi_reset;
// int_axi_referenceValue[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_axi_referenceValue[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_AXI_REFERENCEVALUE_DATA_0)
            int_axi_referenceValue[31:0] <= (WDATA[31:0] & wmask) | (int_axi_referenceValue[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_AXI_KI_DATA_0)
            int_axi_KI[31:0] <= (WDATA[31:0] & wmask) | (int_axi_KI[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_AXI_KP_DATA_0)
            int_axi_KP[31:0] <= (WDATA[31:0] & wmask) | (int_axi_KP[31:0] & ~wmask);
    end
end
