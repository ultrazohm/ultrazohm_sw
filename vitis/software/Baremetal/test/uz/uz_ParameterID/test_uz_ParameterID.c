...
void test_uz_ParameterID_generate_DutyCycle_Data_NULL(void) {
    //PID_Data null pointer will be asserted first, therefore PWM can stay NULL too
    uz_3ph_dq_t input = {0};
...
void test_uz_ParameterID_generate_DutyCycle_PWM_NULL(void) {
    uz_3ph_dq_t input = {0};
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_generate_DutyCycle(&PID_Data, input, NULL));
}
...
void test_uz_ParameterID_Controller_FOC_NULL(void) {
    uz_SpeedControl_t* SC_instance = uz_SpeedControl_init(config_n);
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_Controller(&PID_Data, NULL, SC_instance));
}

void test_uz_ParameterID_Controller_SC_NULL(void) {
    uz_FOC* FOC_instance = uz_FOC_init(config);
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_Controller(&PID_Data, FOC_instance, NULL));
}

void test_uz_ParameterID_CleanPsiArray_NULL(void) {
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_CleanPsiArray(NULL, &PID_Data));
}
...
void test_uz_ParameterID_CalcFluxMaps_NULL(void) {
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_CalcFluxMaps(NULL, &PID_Data));
}
...
void test_uz_ParameterID_update_transmit_values_activeState_NULL(void) {
    float FluxMapCounter = 0.0f;
    float ArrayCounter = 0.0f;
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_update_transmit_values(&PID_Data, NULL, &FluxMapCounter, &ArrayCounter));
}

void test_uz_ParameterID_update_transmit_values_FluxCounter_NULL(void) {
    float activeState = 0.0f;
    float ArrayCounter = 0.0f;
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_update_transmit_values(&PID_Data, &activeState, NULL, &ArrayCounter));
}

void test_uz_ParameterID_update_transmit_values_ArrayCounter_NULL(void) {
    float activeState = 0.0f;
    float FluxMapCounter = 0.0f;
    TEST_ASSERT_FAIL_ASSERT(uz_ParameterID_update_transmit_values(&PID_Data, &activeState, &FluxMapCounter, NULL));
}
