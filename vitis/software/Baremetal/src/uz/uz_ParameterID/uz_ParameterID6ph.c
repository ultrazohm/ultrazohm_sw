...
typedef struct uz_ParameterID6ph_t {
	bool is_ready;
	uz_ParaID_ControlState_t* ControlState6ph;
	uz_ParaID_ElectricalID6ph0_t *ElectricalID6ph;
...
static uint32_t instances_counter_ParameterID6ph = 0;
static uz_ParameterID6ph_t instances_ParameterID6ph[UZ_PARAMETERID6PH_MAX_INSTANCES] = { 0 };

static void uz_ParaID6ph_ControlState_step(uz_ParameterID6ph_t* self, uz_ParameterID6ph_Data_t* Data);
static void uz_ParaID6ph_ElectricalID_step(uz_ParameterID6ph_t* self, uz_ParameterID6ph_Data_t* Data);
...
static void uz_ParameterID6ph_initialize_data_structs(uz_ParameterID6ph_t *self, uz_ParameterID6ph_Data_t *Data);

static uz_ParameterID6ph_t* uz_ParameterID6ph_allocation(void);

static uz_ParameterID6ph_t* uz_ParameterID6ph_allocation(void) {
	uz_assert(instances_counter_ParameterID6ph < UZ_PARAMETERID6PH_MAX_INSTANCES);
	uz_ParameterID6ph_t* self = &instances_ParameterID6ph[instances_counter_ParameterID6ph];
	uz_assert(self->is_ready == false);
	instances_counter_ParameterID6ph++;
	self->is_ready = true;
	return (self);
}
...
uz_ParameterID6ph_t* uz_ParameterID6ph_init(uz_ParameterID6ph_Data_t *Data) {
	...
	uz_ParameterID6ph_t* self = uz_ParameterID6ph_allocation();
	self->ControlState = uz_ControlState6ph_init();
	self->ElectricalID = uz_ElectricalID6ph_init();
	...
	uz_ParameterID6ph_initialize_data_structs(self, Data);
	return (self);
}
...
void uz_ParameterID6ph_step(uz_ParameterID6ph_t* self, uz_ParameterID6ph_Data_t* Data) {
	...
	uz_ParaID6ph_ControlState_step(self, Data);
	...
	if (self->ControlState->output.ControlFlags.finished_all_Offline_states == false) {
		...
		if (self->ControlState->output.ControlFlags.transNr == 1U || self->ControlState->output.GlobalConfig_out.Reset == true) {
			uz_ParaID6ph_ElectricalID_step(self, Data);
...
static void uz_ParaID6ph_ElectricalID_step(uz_ParameterID6ph_t* self, uz_ParameterID6ph_Data_t* Data) {
	...
	//Step the function
	uz_ElectricalID6ph_step(self->ElectricalID);
...
static void uz_ParaID6ph_ControlState_step(uz_ParameterID_t* self, uz_ParameterID_Data_t* Data) {
	uz_assert_not_NULL(self);
	uz_assert_not_NULL(Data);
